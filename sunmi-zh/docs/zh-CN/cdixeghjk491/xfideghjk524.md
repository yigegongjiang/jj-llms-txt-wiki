---
url: https://docs.sunmi.com/zh-CN/cdixeghjk491/xfideghjk524
---

# 3.下载配网SDK（仅适用于使用了SDK_V1接口的旧设备）

更新时间：2025-11-10 16:35:03

# 说明

> 说明：云打印机配网、云绑定过程是通过蓝牙实现的，所以需要利用手机的蓝牙来进行指令的通讯。SDK里的SunmiPrinterClient类对蓝牙配网过程进行了封装，便于APP简单开发实现打印机配对。使用该功能时必须保证蓝牙打开且允许应用获取位置权限。
>
...
在输入框中输入商米提供给您的真实APP\_ID和APP\_KEY，SHOP ID您可以自行定义（小于32位字符），输入完成后点击下方“开始”
...
上一篇：2.注册一个合作伙伴账号（仅适用于使用了SDK_V1接口的旧设备）
下一篇：4. 开发对接API接口（回调方式）（仅适用于使用了SDK_V1接口的旧设备）
