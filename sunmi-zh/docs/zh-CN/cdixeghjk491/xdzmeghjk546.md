---
url: https://docs.sunmi.com/zh-CN/cdixeghjk491/xdzmeghjk546
---

# 打印标签小票接口
更新时间：2025-09-24 16:49:50
## 一、功能介绍
* * *
...
intent.setAction("com.sunmi.printer.settings");
...
| 可用方法  | 方法说明  | 参数说明  | 默认值  |  
| --- | --- | --- | --- |  
| enableReverse  | 标签打印方向设置  | 是否反向  | 打印机默认设置  |  
| enableMirror  | 标签打印镜像设置  | 是否镜像  | 打印机默认设置  |  
| enableBack  | 当打印机支持切刀时每次切刀结束时根据此标志决定下一张标签是否回退当打印机不支持切刀时打印结束后若开启了撕纸标志将决定下一次打印是否回退  | 是否开启  | 打印机默认设置  |  
| enableTear  | 当打印机不支持切刀时决定是否在打印结束后将标签纸进纸到撕纸位置  | 是否开启  | 打印机默认设置  |  
### 2. 绘制文本内容
bash c cpp csharp css go java javascript json kotlin less makefile markdown objectivec php plaintext python rust scss shell sql swift typescript xml yaml
...
| setAlign  | 设置文本内容相对起始坐标的位置  | [见Align](https://docs.sunmi.com/zh-CN/cdixeghjk491/xdzmeghjk546#enum1)  | Align.DEFAUL  |  
| setRotate  | 设置文本内容的方向  | [见Rotate](https://docs.sunmi.com/zh-CN/cdixeghjk491/xdzmeghjk546#enum2)  | 水平方向  |  
...
| setAlign  | 设置条码相对起始坐标位置  | [见Align](https://docs.sunmi.com/zh-CN/cdixeghjk491/xdzmeghjk546#enum1)  | Align.DEFAUL  |  
| setRotate  | 设置条码旋转方向  | [见Rotate](https://docs.sunmi.com/zh-CN/cdixeghjk491/xdzmeghjk546#enum2)  | 水平方向  |  
...
最终将影响二维码大小  | 4  |  
| setErrorLevel  | 设置二维码纠错等级  | [见ErrorLevel](https://docs.sunmi.com/zh-CN/cdixeghjk491/xdzmeghjk546#enum5)  | ErrorLevel.L  |  
| setRotate  | 设置二维码旋转方向  | [见Rotate](https://docs.sunmi.com/zh-CN/cdixeghjk491/xdzmeghjk546#enum2)  | 水平方向  |  
...
| setAlgorithm  | 设置图片转换方式  | [见ImageAlgorithm](https://docs.sunmi.com/zh-CN/cdixeghjk491/xdzmeghjk546#enum6)  | ImageAlgorithm.BINARIZATION  |  
| setValue  | 设置算法浮动值  | 根据具体的算法浮动值不同  | [见ImageAlgorithm](https://docs.sunmi.com/zh-CN/cdixeghjk491/xdzmeghjk546#enum6)  |  
...
| 可用方法  | 方法说明  | 参数说明  | 默认值  |  
| --- | --- | --- | --- |  
| setStyle  | 设置绘制形状  | [见Shape](https://docs.sunmi.com/zh-CN/cdixeghjk491/xdzmeghjk546#enum7)  | Shape.RECT_FILL  |  
| setWidth  | 设置图形的宽度   
...
**PrintResult 说明**
resultCode 打印成功返回0 打印失败返回 <0
message 失败时附加信息
商米内置打印机可以获取打印机的真实打印结果；
对外接的打印机无法获得可考的打印结果；
## 四、综合示例
构建一个完整的由标题+二维码+文本+条码的样式模板，用于标签打印使用
bash c cpp csharp css go java javascript json kotlin less makefile markdown objectivec php plaintext python rust scss shell sql swift typescript xml yaml
...
printer.canvasApi().renderArea(
                    ...
                    .setWidth(330)
                    .setHeight(330));
printer.canvasApi().renderText("这是标题",
                    TextStyle.getStyle()
                    .setPosX(165)
                    .setPosY(5)
                    .setAlign(Align.CENTER));
printer.canvasApi().renderQrCode("123456789",
                    QrStyle.getStyle()
                    .setPosX(5)
                    .setPosY(30).
                    setWidth(130)
                    .setHeight(130));
printer.canvasApi().renderText("商米是一家以“利他心”为核心价值观，" +
                            "全面引领全球智能商用硬件创新的物联网科技公司。",
                    TextStyle.getStyle()
                    .setPosX(140)
                    .setPosY(30)
                    .setWidth(180)
                    .setHeight(120));
printer.canvasApi().renderBarCode("12345678",
...
### 2. 需要处理标签待取走的状态
在一些自带标签打印设备上如S2LCC机型支持检测标签纸是否未取走，因为未取走状态不会影响标签连续打印的进度，此时业务软件可以根据需要通过代码控制检测标签取走的情况
如果不关心标签是否取走的情况，就和一般打印方式一样通过printCanvas(n, null)一次可以打印N张标签
如果需要处理标签是否取走（只有取走后才可以打印下一张标签的业务场景）可通过如下代码实现
bash c cpp csharp css go java javascript json kotlin less makefile markdown objectivec php plaintext python rust scss shell sql swift typescript xml yaml

```
... 绘制标签内容
printer.canvasApi().printCanvas(1, new PrintResult() {
                @Override
                public void onResult(int resultCode, String message) throws RemoteException {
                    监听打印标签的结果，当结果异常时通过queryApi获取打印机的实时状态，根据返回状态实现不同的业务需求
                    如标签待取走状态，可在此提示使用者及时拿走标签
                    当结果非异常可以决定是否打印下一张标签
                }
            );
...
打印机标签取走以及状态的恢复均可通过监听：woyou.aidlservice.jiuv5.NORMAL_ACTION 广播获取
* * *
## 六、枚举参数说明
### Align 对齐方式  
| 枚举内容  | 说明  |  
