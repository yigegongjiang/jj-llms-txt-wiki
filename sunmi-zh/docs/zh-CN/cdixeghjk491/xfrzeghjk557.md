---
url: https://docs.sunmi.com/zh-CN/cdixeghjk491/xfrzeghjk557
---

# 14、云打印机iOS SDK多台同时连接
更新时间：2025-12-19 22:50:57
本功能仅支持以下机型和版本：
...
# SDK DEMO
SDK for iOS代码下载：
![](data:image/svg+xml,%3csvg%20width='16'%20height='16'%20viewBox='0%200%2016%2016'%20fill='none'%20xmlns='http://www.w3.org/2000/svg'%3e%3cpath%20d='M4.5%203C4.5%201.61929%205.61929%200.5%207%200.5C8.38071%200.5%209.5%201.61929%209.5%203V12C9.5%2012.8284%208.82843%2013.5%208%2013.5C7.17157%2013.5%206.5%2012.8284%206.5%2012V5C6.5%204.72386%206.72386%204.5%207%204.5C7.27614%204.5%207.5%204.72386%207.5%205V12C7.5%2012.2761%207.72386%2012.5%208%2012.5C8.27614%2012.5%208.5%2012.2761%208.5%2012V3C8.5%202.17157%207.82843%201.5%207%201.5C6.17157%201.5%205.5%202.17157%205.5%203V12C5.5%2013.3807%206.61929%2014.5%208%2014.5C9.38071%2014.5%2010.5%2013.3807%2010.5%2012V5C10.5%204.72386%2010.7239%204.5%2011%204.5C11.2761%204.5%2011.5%204.72386%2011.5%205V12C11.5%2013.933%209.933%2015.5%208%2015.5C6.067%2015.5%204.5%2013.933%204.5%2012V3Z'%20fill='black'/%3e%3c/svg%3e) sunmi_printer_ipad.zip 
# SDK快速入门
  1. Print SDK for iOS下载：
![](data:image/svg+xml,%3csvg%20width='16'%20height='16'%20viewBox='0%200%2016%2016'%20fill='none'%20xmlns='http://www.w3.org/2000/svg'%3e%3cpath%20d='M4.5%203C4.5%201.61929%205.61929%200.5%207%200.5C8.38071%200.5%209.5%201.61929%209.5%203V12C9.5%2012.8284%208.82843%2013.5%208%2013.5C7.17157%2013.5%206.5%2012.8284%206.5%2012V5C6.5%204.72386%206.72386%204.5%207%204.5C7.27614%204.5%207.5%204.72386%207.5%205V12C7.5%2012.2761%207.72386%2012.5%208%2012.5C8.27614%2012.5%208.5%2012.2761%208.5%2012V3C8.5%202.17157%207.82843%201.5%207%201.5C6.17157%201.5%205.5%202.17157%205.5%203V12C5.5%2013.3807%206.61929%2014.5%208%2014.5C9.38071%2014.5%2010.5%2013.3807%2010.5%2012V5C10.5%204.72386%2010.7239%204.5%2011%204.5C11.2761%204.5%2011.5%204.72386%2011.5%205V12C11.5%2013.933%209.933%2015.5%208%2015.5C6.067%2015.5%204.5%2013.933%204.5%2012V3Z'%20fill='black'/%3e%3c/svg%3e) SMPrinterManager.xcframework.zip 
  2. 目前 SDK 只支持手动集成
  3. 将 `SMPrinterManager.framework` 导入到 Xcode 工程目录下
  4. 在需要使用 API 的类中添加 `#import <SMPrinterManager/SMPrinterManager.h>` 头文件
...
6.使用 TCP/IP 局域网打印机，需要增加iOS网络发现权限控制。
1) 申请网络发现权限：<https://developer.apple.com/contact/request/networking-multicast>（申请大概需要3天左右，成功后会有邮件回复）
2) 申请成功后，在开发者账号中，对【appid】进行编辑，找到【Additional Capabilities】选项，勾选【Multicast Networking】
3) 在 app 的【xxxx.entitlements】配置文件中添加 `com.apple.developer.networking.multicast` ，值设置 `YES`
4) 在 info.plist 中添加本地网络权限
bash c cpp csharp css go java javascript json kotlin less makefile markdown objectivec php plaintext python rust scss shell sql swift typescript xml yaml
...
| **属性**  | **类型**  | **描述**  |  
| --- | --- | --- |  
| isConnected  | BOOL  | 当前蓝牙打印机的连接状态  |  
| rssi  | NSNumber  | 发现蓝牙打印机时取到的信号强度值  |  
| peripheral  | CBPeripheral  | 蓝牙打印机设备  |  
| deviceName  | NSString  | 蓝牙打印机名称  |  
| **方法**  | **描述**  |  
| --- | --- |  
| - (**instancetype**)initWithPeripheral:(CBPeripheral  _)peripheral isConnected:(_**_BOOL_** _)isConnected rssi:(NSNumber_)rssi;  | 初始化 SMBleDevice 实例  |  
| - (**void**)discoverServices;  | 搜索蓝牙打印机服务和特征值  |  
| - (**void**)controlDevicePrintingData:(NSData *)data;  | 向蓝牙打印机发送数据  |  
| - (**void**)controlDevicePrintingData:(NSData *)data success:(SuccessBlock)success fail:(FailureBlock)fail;  | 向蓝牙打印机发送数据，发送数据成功、失败回调  |  
| - (**void**)controlDevicePrintingData:(NSData *)data success:(SuccessBlock)success fail:(FailureBlock)fail response:(ResponseBlock)response;  | 向蓝牙打印机发送数据，发送数据成功、失败回调以及返回相应数据  |  
### **SMBleConnectManage**  
| **属性**  | **描述**  |  
| --- | --- |  
| smBleDevices  | 搜索到的蓝牙打印机列表  |  
| delegate  | 相关代理协议  |  
| **方法**  | **描述**  |  
| --- | --- |  
| + (**instancetype**)shareInstance;  | 初始化蓝牙管理类单例实例  |  
| + (**void**)destroyInstance;  | 销毁实例  |  
| - (**void**)startScanPeripheral;  | 开始搜索蓝牙打印机  |  
| - (**void**)stopScanPeripheral;  | 停止搜索蓝牙打印机  |  
| - (**void**)connectWithDevice:(SMBleDevice *)device;  | 连接指定的蓝牙打印机  |  
| - (**void**)disconnectWithDevice:(SMBleDevice *)device;  | 断开指定的蓝牙打印机  |  
### **BlePrinterManagerDelegate**  
| **代理方法**  | **描述**  |  
| --- | --- |  
| - (**void**)updateBluetoorhDevices;  | 打印机列表状态发生改变后，执行代理方法（重写此代理方法更新 UI）  |  
# 局域网方式连接多台打印机
### **涉及到的类**
bash c cpp csharp css go java javascript json kotlin less makefile markdown objectivec php plaintext python rust scss shell sql swift typescript xml yaml
...
| **属性**  | **类型**  | **描述**  |  
| --- | --- | --- |  
| tcpSocket  | GCDAsyncSocket  | 局域网设备连接的 socket  |  
| deviceIP  | NSString  | 打印机的 ip 地址  |  
| deviceSN  | NSString  | 打印机的 SN  |  
| deviceMode  | NSString  | 打印机的类型  |  
| isConnected  | **BOOL**  | 当前是否连接状态  |  
| **方法**  | **描述**  |  
| --- | --- |  
| - (**id**)initWithDeviceIP:(NSString  _)deviceIP deviceSN:(NSString_)deviceSN deviceModel:(NSString *)deviceMode isConnected:(**BOOL**)isConnected;  | 初始化 SMIPDevice 实例  |  
| - (**void**)controlDevicePrintingData:(NSData *)ipData;  | 向打印机发送数据  |  
| - (**void**)controlDevicePrintingData:(NSData *)ipData success:(SuccessBlock)success fail:(FailureBlock)fail;  | 向打印机发送数据，发送数据成功、失败回调  |  
| - (**void**)controlDevicePrintingData:(NSData *)ipData success:(SuccessBlock)success fail:(FailureBlock)fail response:(ResponseBlock)response;  | 向打印机发送数据，发送数据成功、失败回调以及返回相应数据  |  
| - (**void**)handleReceivedSocket:(GCDAsyncSocket  _)sock data:(NSData_)data withTag:(**long**)tag;  | 写入、读取数据使用（内部使用）  |  
### **SMIPConnectManager**  
| **属性**  | **描述**  |  
| --- | --- |  
| smIPDevices  | 搜索到同一局域网内的打印机列表  |  
| delegate  | 相关代理协议  |  
| **方法**  | **描述**  |  
| --- | --- |  
| + (**instancetype**)sharedInstance;  | 初始化局域网管理类单例实例  |  
| + (**void**)destroyInstance;  | 销毁实例  |  
| - (**void**)startSearchWithIP:(NSString *)searchIPString;  | 开始搜索局域网内的打印机  |  
| - (**void**)stopSearching;  | 停止搜索局域网内的打印机  |  
| - (**void**)connectWithDevice:(SMIpDevice *)device;  | 连接指定的局域网打印机  |  
| - (**void**)disconnectWithDevice:(SMIpDevice *)device;  | 断开指定的局域网打印机  |  
### **IPPrinterManagerDelegate**  
| **代理方法**  | **描述**  |  
| --- | --- |  
| - (**void**)updateIPDevices;  | 局域网打印机列表状态发生改变后，执行代理方法（重写此代理方法更新 UI）  |  
# 蓝牙配网模块
### **SMBleNetworkManager**  
| **方法**  | **描述**  |  
| --- | --- |  
| + (**instancetype**)shareInstance;  | 初始化蓝牙配网管理类实例  |  
| + (**void**)destroyInstance;  | 销毁蓝牙配网管理类实例  |  
| - (**void**)startScanPeripheral;  | 开始扫描蓝牙打印机设备  |  
| - (**void**)stopScanPeripheral;  | 停止扫描蓝牙打印机设备  |  
| - (**void**)connectPeripheral:(CBPeripheral *)peripheral;  | 连接指定的蓝牙打印机设备  |  
| - (**void**)disConnectPeripheral;  | 断开蓝牙打印机设备  |  
| - (**void**)enterNetworkMode;  |  向蓝牙打印机发送自动进入配网模式的指令。 手动配网需要长安配网键（此方法不需要调用）  |  
| - (**void**)getWifiList;  | 向蓝牙打印机发送获取 Wi-Fi 列表的指令  |  
| - (**void**)connectAP:(NSString  _)ssid password:(NSString_)password;  | 向蓝牙打印机发送连接 Wi-Fi 网络的 ssid 和 password 的指令  |  
| - (**void**)quitConnectAP;  | 向蓝牙打印机发送退出配网模式的指令  |  
| - (**void**)deleteWifiSetting;  | 向蓝牙打印机发送删除 Wi-Fi 配置的指令  |  
### **BleNetworkDelegate**  
| **方法**  | **描述**  |  
| --- | --- |  
| - (**void**)discoveredDevice:(CBPeripheral *)device;  | 扫描到打印机时，可以在此协议方法中获取扫描到的设备，可能会执行多次，直到扫描结束  |  
| - (**void**)didFinishSearching;  | 扫描打印机结束的代理方法，可以在此协议方法中做连接后的逻辑  |  
| - (**void**)didConectPrinter;  | 成功连接到打印机，可以在此协议方法中做连接后的逻辑  |  
| - (**void**)didEnterNetworkMode;  | 打印机已经进入配网模式，可以在此协议方法中做连接后的逻辑  |  
| - (**void**)receiveAPInfo:(NSDictionary *)apInfo;  | 获取打印机返回的 Wi-Fi 信息，会执行多次  |  
| - (**void**)receiveApInfoCompleted;  | Wi-Fi 信息接收完毕，会执行此协议方法  |  
| - (**void**)receiveApInfoFailed:(NSError *)error;  | 获取打印机 Wi-Fi 信息失败，在此协议方法内可做其他的自定义处理  |  
| - (**void**)configPrinterApSuccess;  | 配网成功，可以在此协议方法内做自定义的操作  |  
| - (**void**)configPrinterApFailed:(NSError *)error;  | 配网失败，可以在此协议方法内做自定义的操作  |  
| - (**void**)didFailConnectDevice:(CBPeripheral  _)device error:(NSError_)error;  | 连接到打印机失败，并返回失败原因。可以在此协议方法中做连接失败后的操作  |  
| - (**void**)didDisconnectDevice:(CBPeripheral  _)device error:(NSError_)error;  | 设备主动断开，并返回断开原因。可以在此协议方法中做断开后的操作  |  
# 打印机相关指令
bash c cpp csharp css go java javascript json kotlin less makefile markdown objectivec php plaintext python rust scss shell sql swift typescript xml yaml
...
```
SMPrinterCommand *command = [[SMPrinterCommand alloc] init];
[command setUtf8Mode:1];
[command appendText:@"测试文本1"];
[command horizontalTab:1];
[command appendText:@"测试文本2"];
[command lineFeed:1];
[[SMBleConnectManage shareInstance] sendPrintData:[command getCommandData]];
```
...
SMPrinterCommand *command = [[SMPrinterCommand alloc] init];
[command setUtf8Mode:1];
SMColumnSetting *set1 = [[SMColumnSetting alloc] init];
set1.width = 192;
set1.alignment = SMAlignStyle_Left;
set1.mode = SMPrintModel_Original;
SMColumnSetting *set2 = [[SMColumnSetting alloc] init];
set2.width = 192;
set2.alignment = SMAlignStyle_Center;
set2.mode = SMPrintModel_Original;
SMColumnSetting *set3 = [[SMColumnSetting alloc] init];
set3.width = 192;
...
```
@interface SMColumnSetting : NSObject
...
```
- (void)setupColumns:(NSArray <SMColumnSetting*>*)settings;
```
  
| **参数名**  | **类型**  | **说明**  |  
| --- | --- | --- |  
| settings  | NSArray  | 设置分列打印的配置，数组的元素为 SMColumnSetting，详见 SMColumnSetting  |  
  * **设置分列打印文本**
...
| SMBarcodeType_CODE39  | NSInteger  | Code39条码生成字符集包括数字 、大写字母以及- . $ / + % _空格等字符其中"_ "只用于标记开始和结束  |  
