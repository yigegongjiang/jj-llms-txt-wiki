---
url: https://docs.sunmi.com/zh-CN/cdixeghjk491/xdireghjk568
---

# JavaScript 打印热敏小票接口
更新时间：2025-09-24 17:57:24
## 一、功能介绍
...
> initLine: (baseStyle: BaseStyle) => Promise
> addText: (text: string, textStyle: TextStyle) => Promise
> printText: (text: string, textStyle: TextStyle) => Promise
> printTexts: (text: string[], colsWidthArr: number[], textStyle: TextStyle[]) => Promise
> printBarCode: (code: string, style: BarcodeStyle) => Promise
> printQrCode: (code: string, qrStyle: QrStyle) => Promise
> printBitmap: (bitmap: string, style: BitmapStyle) => Promise
> printDividingLine: (style: DividingLine, offset: number) => Promise
> autoOut: () => Promise
> enableTransMode:(enable: boolean) => Promise
> printTrans: () => Promise
## 二、接口说明
### 1. 行设置
bash c cpp csharp css go java javascript json kotlin less makefile markdown objectivec php plaintext python rust scss shell sql swift typescript xml yaml
...
```
//设置默认初始化
printer.lineApi.initLine(BaseStyle.getStyle()).then((res) => {});
//设置初始化对齐居中
printer.lineApi.initLine(BaseStyle.getStyle().setAlign(Align.CENTER)).then((res) => {});
...
| setEnableBold  | 文本加粗  | 开启文本加粗功能  | 不开启  |  
| setEnableUnderline  | 文本下划线  | 开启文本下划线功能  | 不开启  |  
| setEnableStrikethrough  | 文本删除线  | 开启文本删除线功能  | 不开启  |  
| setEnableItalics  | 文本斜体  | 开启文本斜体功能  | 不开启  |  
| setEnableInvert  | 文本倒置  | 开启文本倒置功能  | 不开启  |  
| setEnableAntiColor  | 文本反白  | 开启文本反白功能  | 不开启  |  
...
```
printer.lineApi.addText("测试内容1", TextStyle.getStyle).then((res) => {});
printer.lineApi.addText(" 测试内容2", TextStyle.getStyle).then((res) => {});
printer.lineApi.addText("测试内容3<br></br>", TextStyle.getStyle).then((res) => {});
printer.lineApi.printText("测试内容4", TextStyle.getStyle).then((res) => {});
...
```
printer.lineApi.printTexts(
          ["Default Text", "Default Text"],
          [1, 2],
          [TextStyle.getStyle(), TextStyle.getStyle()]).then((res) => {});
...
```
printer.lineApi.printBarCode(
          "1234567890",
          BarcodeStyle.getStyle().setSymbology(Symbology.CODABAR)
        ).then((res) => {});
...
```
printer.lineApi.printQrCode(content, QrStyle.getStyle()).then((res) => {});
...
```
.printBitmap(
"",
...
```
.printer.lineApi.printDividingLine(DividingLine.SOLID, 30).then((res) => {});
...
```
printer.lineApi.autoOut().then((res) => {});
...
```
printer.lineApi.enableTransMode(true).then((res) => {
        const data = JSON.stringify(res);
        Toast(data);
        renderSocketResponse(data || "Success");
        printer.lineApi
          .addText("Hello, World addText1", TextStyle.getStyle())
          .then((res) => {
            const data = JSON.stringify(res);
            Toast(data);
            renderSocketResponse(data || "Success");
            printer.lineApi
              .addText("Hello, World addText2", TextStyle.getStyle())
              .then((res) => {
                const data = JSON.stringify(res);
                Toast(data);
                renderSocketResponse(data || "Success");
                printer.lineApi
                  .printText("Hello, World! printText", TextStyle.getStyle())
                  .then((res) => {
                    const data = JSON.stringify(res);
                    Toast(data);
                    renderSocketResponse(data || "Success");
                    printer.lineApi.printTrans().then((res) => {
                      const data = JSON.stringify(res);
                      Toast(data);
                      renderSocketResponse(data || "Success");
                      // 关闭事物模式
...
上一篇：JavaScript SDK概览
下一篇：JavaScrip 打印标签小票接口
