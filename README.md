...
- **`URL:` 标记式**：H1 + 可选 blockquote + 独立 `URL: <URL>`；其前可有 `---`。页头是页面内容，标题和描述保留进输出，仅移除 `URL:` 元数据行。标记值为绝对 HTTP(S) URL，或相对 URL（如 Fumadocs 的 `URL: /docs/x`）——相对值按入口文档的最终 URL（跟随重定向后）解析，解析结果仍须是 HTTP(S)。能独立解析的绝对 URL 一律不重定基，`file:` / `mailto:` 等照常报错。
