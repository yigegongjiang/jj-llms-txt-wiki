...
> Unlink a local JSR package from the current project
...
[`links`](/runtime/reference/deno_json/#overriding-packages) array in the
...
[`deno link`](/runtime/reference/cli/link/) resolves them, so
`deno unlink
../pkg` matches an entry created by `deno link ../pkg` from the
