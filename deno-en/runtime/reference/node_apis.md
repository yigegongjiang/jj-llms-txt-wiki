...
| [`__dirname`](https://nodejs.org/api/globals.html#__dirname)                                                     | ⚠️ [Info](#__dirname-and-__filename) |
| [`__filename`](https://nodejs.org/api/globals.html#__filename)                                                   | ⚠️ [Info](#__dirname-and-__filename) |
...
`__dirname` and `__filename` are CommonJS globals. They are available inside
CommonJS modules in the `npm` package scope, but they are not defined in ES
modules. In your own ES modules use `import.meta.dirname` and
`import.meta.filename` instead, which provide the same values for the current
module.
