...
Last updated Aug 10, 2026|Copy as Markdown|[View as Markdown](https://developers.cloudflare.com/d1/best-practices/use-indexes/index.md)|[Agent setup](https://developers.cloudflare.com/agent-setup/)
...
* For columns used to join tables together - for example, the `ON orders.customer_id = customers.id` in a `JOIN`. A suitable index can avoid repeated scans or the creation of a temporary index. Use [EXPLAIN QUERY PLAN](#test-an-index) to verify the query plan.
...
## Indexes and your bill

D1 [bills by the number of rows read and rows written](https://developers.cloudflare.com/d1/platform/pricing/), not by the number of rows your query returns. A query that scans an entire table to return a single row is billed for every row it scans. Adding an index so that D1 can jump straight to the rows it needs is therefore one of the most effective ways to reduce both latency **and** cost.

Most unexpectedly large D1 bills come from a small number of frequently run queries that each read (or write) far more rows than they return. The following table lists common patterns that touch too many rows, along with how to fix them. Use [EXPLAIN QUERY PLAN](#test-an-index) to check whether a query does a full `SCAN` (reads every row) or a `SEARCH ... USING INDEX` (jumps to matching rows).

| Pattern                                                                               | Why it reads or writes too many rows                                                                                                                                     | Fix                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| WHERE column = ? on an unindexed column                                               | D1 scans the whole table on every call.                                                                                                                                  | Create an index on the filtered column.                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| WHERE a = ? AND b = ? with no index on either column                                  | D1 scans the whole table. An index on just one of the two columns avoids the full scan, but D1 still reads every row matching that column before filtering on the other. | Create a [multi-column index](#multi-column-indexes) on (a, b) so D1 can narrow on both columns at once.                                                                                                                                                                                                                                                                                                                                                                                            |
...
* This would allow queries against orders that are unfulfilled, in-progress, or shipped, which are likely to be some of the most common queries (users checking their order status).
...
* Indexes cannot be updated. To add or remove a column from an index, [remove](#remove-indexes) the index and then [create a new index](#create-an-index) with the new columns. Apply these schema changes once through a versioned system such as [D1 migrations](https://developers.cloudflare.com/d1/reference/migrations/).
