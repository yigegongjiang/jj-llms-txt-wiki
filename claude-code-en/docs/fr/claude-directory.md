...
Sur Windows, `~/.claude` se résout en `%USERPROFILE%\.claude`. Si vous définissez [`CLAUDE_CONFIG_DIR`](/docs/fr/env-vars), chaque chemin `~/.claude` sur cette page se trouve sous ce répertoire à la place.
...
| ----------------------- | ----------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `managed-settings.json` | Au niveau du système, varie selon le système d'exploitation | Paramètres appliqués par l'entreprise que vous ne pouvez pas remplacer. Consultez [paramètres gérés par le serveur](/docs/fr/server-managed-settings).                                                                                                                                                                        |
...
| :---------------------------------------------------------------- | :--------------------------------------- | :---------------- | :---------------------------------------------------- |
| Donner à Claude le contexte et les conventions du projet          | `CLAUDE.md`                              | projet ou global  | [Mémoire](/docs/fr/memory)                                 |
| Autoriser ou bloquer des appels d'outils spécifiques              | `settings.json` `permissions` ou `hooks` | projet ou global  | [Permissions](/docs/fr/permissions), [Hooks](/docs/fr/hooks)    |
| Exécuter un script avant ou après les appels d'outils             | `settings.json` `hooks`                  | projet ou global  | [Hooks](/docs/fr/hooks)                                    |
| Définir les variables d'environnement pour la session             | `settings.json` `env`                    | projet ou global  | [Paramètres](/docs/fr/settings#available-settings)         |
| Garder les remplacements personnels hors de git                   | `settings.local.json`                    | projet uniquement | [Portées des paramètres](/docs/fr/settings#settings-files) |
| Ajouter une invite ou une capacité que vous invoquez avec `/name` | `skills/<name>/SKILL.md`                 | projet ou global  | [Skills](/docs/fr/skills)                                  |
| Définir un subagent spécialisé avec ses propres outils            | `agents/*.md`                            | projet ou global  | [Subagents](/docs/fr/sub-agents)                           |
| Orchestrer de nombreux subagents à partir d'un script             | `workflows/*.js`                         | projet ou global  | [Workflows dynamiques](/docs/fr/workflows)                 |
| Connecter des outils externes via MCP                             | `.mcp.json`                              | projet uniquement | [MCP](/docs/fr/mcp)                                        |
| Modifier la façon dont Claude formate les réponses                | `output-styles/*.md`                     | projet ou global  | [Styles de sortie](/docs/fr/output-styles)                 |
...
<Note>
  ...
  * [Les paramètres gérés](/docs/fr/server-managed-settings) déployés par votre organisation ont la priorité sur tout
  * Les flags CLI comme `--permission-mode` ou `--settings` remplacent `settings.json` pour cette session
  * Certaines variables d'environnement ont la priorité sur leur paramètre équivalent, mais cela varie : consultez la [référence des variables d'environnement](/docs/fr/env-vars) pour chacune

  Consultez [précédence des paramètres](/docs/fr/settings#settings-precedence) pour l'ordre complet.
</Note>
...
| --------------------------------------------------- | ----------------- | ------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------- |
| [`CLAUDE.md`](#ce-claude-md)                        | Projet et global  | ✓       | Instructions chargées à chaque session                                                                                                          | [Mémoire](/docs/fr/memory)                                             |
| [`rules/*.md`](#ce-rules)                           | Projet et global  | ✓       | Instructions limitées à un sujet, optionnellement limitées par chemin                                                                           | [Rules](/docs/fr/memory#organize-rules-with-claude/rules/)             |
| [`settings.json`](#ce-settings-json)                | Projet et global  | ✓       | Permissions, hooks, variables d'environnement, paramètres par défaut du modèle                                                                  | [Paramètres](/docs/fr/settings)                                        |
| [`settings.local.json`](#ce-settings-local-json)    | Projet uniquement |         | Vos remplacements personnels, auto-gitignorés                                                                                                   | [Portées des paramètres](/docs/fr/settings#settings-files)             |
| [`.mcp.json`](#ce-mcp-json)                         | Projet uniquement | ✓       | Serveurs MCP partagés par l'équipe                                                                                                              | [Portées MCP](/docs/fr/mcp#mcp-installation-scopes)                    |
| [`.worktreeinclude`](#ce-worktreeinclude)           | Projet uniquement | ✓       | Fichiers gitignorés à copier dans les nouveaux worktrees                                                                                        | [Worktrees](/docs/fr/worktrees#copy-gitignored-files-into-worktrees)   |
| [`skills/<name>/SKILL.md`](#ce-skills)              | Projet et global  | ✓       | Invites réutilisables invoquées avec `/name` ou auto-invoquées                                                                                  | [Skills](/docs/fr/skills)                                              |
| [`commands/*.md`](#ce-commands)                     | Projet et global  | ✓       | Invites sur un seul fichier ; même mécanisme que les skills                                                                                     | [Skills](/docs/fr/skills)                                              |
| [`output-styles/*.md`](#ce-output-styles)           | Projet et global  | ✓       | Sections de système-prompt personnalisées                                                                                                       | [Styles de sortie](/docs/fr/output-styles)                             |
| [`agents/*.md`](#ce-agents)                         | Projet et global  | ✓       | Définitions de subagents avec leur propre invite et outils                                                                                      | [Subagents](/docs/fr/sub-agents)                                       |
| [`workflows/*.js`](#ce-workflows)                   | Projet et global  | ✓       | Scripts de flux de travail dynamiques écrits par Claude et enregistrés à partir de `/workflows` ; chaque fichier devient une commande `/<name>` | [Flux de travail dynamiques](/docs/fr/workflows)                       |
| [`agent-memory/<name>/`](#ce-agent-memory)          | Projet et global  | ✓       | Mémoire persistante pour les subagents                                                                                                          | [Mémoire persistante](/docs/fr/sub-agents#enable-persistent-memory)    |
| [`~/.claude.json`](#ce-claude-json)                 | Global uniquement |         | État de l'application, OAuth, bascules d'interface utilisateur, serveurs MCP personnels                                                         | [Configuration globale](/docs/fr/settings#global-config-settings)      |
| [`projects/<project>/memory/`](#ce-global-projects) | Global uniquement |         | Auto memory : notes de Claude à lui-même entre les sessions                                                                                     | [Auto memory](/docs/fr/memory#auto-memory)                             |
| [`keybindings.json`](#ce-keybindings)               | Global uniquement |         | Raccourcis clavier personnalisés                                                                                                                | [Keybindings](/docs/fr/keybindings)                                    |
| [`themes/*.json`](#ce-themes)                       | Global uniquement |         | Thèmes de couleurs personnalisés                                                                                                                | [Thèmes personnalisés](/docs/fr/terminal-config#create-a-custom-theme) |

<h2 id="troubleshoot-configuration">
  Dépannez votre configuration
</h2>

Si un paramètre, un hook ou un fichier ne prend pas effet, consultez [Déboguez votre configuration](/docs/fr/debug-your-config) pour les commandes d'inspection et un tableau de recherche basé sur les symptômes.
...
Les fichiers dans les chemins ci-dessous sont supprimés au démarrage une fois qu'ils sont plus anciens que [`cleanupPeriodDays`](/docs/fr/settings#available-settings). La valeur par défaut est 30 jours.
...
| `projects/<project>/<session>/subagents/`    | Transcriptions de conversation des [sous-agents](/docs/fr/sub-agents), supprimées avec la transcription de session parente lorsqu'elle expire                                                                                                                                                                                                     |
| `projects/<project>/<session>/tool-results/` | Les grandes sorties d'outils sont versées dans des fichiers séparés                                                                                                                                                                                                                                                                          |
| `file-history/<session>/`                    | Instantanés pré-édition des fichiers que Claude a modifiés, utilisés pour [restauration de checkpoint](/docs/fr/checkpointing). Contient des instantanés pour les 100 checkpoints les plus récents ; les fichiers d'instantanés auxquels aucun checkpoint conservé ne fait référence sont supprimés, sauf le premier instantané de chaque fichier |
| `plans/`                                     | Fichiers de plan écrits pendant le [mode plan](/docs/fr/permission-modes#analyze-before-you-edit-with-plan-mode)                                                                                                                                                                                                                                  |
...
| `shell-snapshots/`                           | Alias, fonctions et options shell capturés au démarrage et appliqués par l'[outil Bash](/docs/fr/tools-reference#bash-tool-behavior) à chaque commande. Supprimé à la fermeture correcte. Le balayage efface tout ce qui reste après un crash.                                                                                                    |
| `backups/`                                   | Copies horodatées de `~/.claude.json` prises avant les migrations de configuration                                                                                                                                                                                                                                                           |
| `feedback-bundles/`                          | Archives de transcription expurgées écrites par `/feedback` sur les fournisseurs tiers ou lorsqu'aucune credential Anthropic n'est configurée, pour envoi à votre équipe de compte Anthropic                                                                                                                                                 |
...
| `remote-settings.json`   | Copie en cache des [paramètres gérés par le serveur](/docs/fr/server-managed-settings) pour votre organisation. Présent uniquement lorsque votre organisation les a configurés. Actualisé à chaque lancement. |
...
* Définissez la variable d'environnement [`CLAUDE_CODE_SKIP_PROMPT_HISTORY`](/docs/fr/env-vars) pour ignorer l'écriture des transcriptions et de l'historique des invites dans n'importe quel mode. En mode non interactif, vous pouvez à la place passer `--no-session-persistence` aux côtés de `-p`, ou définir `persistSession: false` dans le SDK Agent.
* Utilisez les [règles de permissions](/docs/fr/permissions) pour refuser les lectures des fichiers de credentials
...
* [Gérez la mémoire de Claude](/docs/fr/memory) : écrivez et organisez CLAUDE.md, rules et auto memory
* [Configurez les paramètres](/docs/fr/settings) : définissez les permissions, hooks, variables d'environnement et paramètres par défaut du modèle
* [Créez des skills](/docs/fr/skills) : créez des invites et des workflows réutilisables
* [Configurez les subagents](/docs/fr/sub-agents) : définissez des agents spécialisés avec leur propre contexte
