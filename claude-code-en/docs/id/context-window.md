...
Jendela konteks Claude Code menyimpan semua yang Claude ketahui tentang sesi Anda: instruksi Anda, file yang dibacanya, respons miliknya sendiri, dan konten yang tidak pernah muncul di terminal Anda. Garis waktu di bawah ini memainkan sesi lengkap dari startup hingga pemadatan: apa yang dimuat sebelum Anda mengetik, apa yang ditambahkan setiap pembacaan file, aturan, dan hook saat Claude bekerja, dan bagaimana subagent menjaga pembacaan besar tetap berada di luar konteks Anda. Lihat [penjelasan tertulis](#what-the-timeline-shows) untuk konten yang sama dalam bentuk daftar.
...
Claude Code secara otomatis dipadatkan saat Anda mendekati batas, sehingga jendela konteks yang penuh tidak mengakhiri sesi Anda. Lintasan otomatis bekerja dengan cara yang sama seperti langkah `/compact` dalam garis waktu. Lihat [Ketika konteks penuh](/docs/id/how-claude-code-works#when-context-fills-up) untuk apa yang dipertahankannya.
...
* [Perluas Claude Code](/docs/id/features-overview): kapan menggunakan CLAUDE.md vs skill vs aturan vs hook vs MCP
* [Simpan instruksi dan memori](/docs/id/memory): hierarki CLAUDE.md dan memori otomatis
* [Subagent](/docs/id/sub-agents): delegasikan penelitian ke jendela konteks terpisah
* [Praktik terbaik](/docs/id/best-practices): mengelola konteks sebagai batasan utama Anda
* [Prompt caching](/docs/id/prompt-caching): tindakan mana yang membatalkan awalan yang di-cache
* [Kurangi penggunaan token](/docs/id/costs#reduce-token-usage): strategi untuk menjaga penggunaan konteks tetap rendah
