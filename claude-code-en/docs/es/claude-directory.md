...
> Dónde Claude Code lee CLAUDE.md, settings.json, hooks, skills, commands, subagents, workflows, rules y auto memory. Explore el directorio .claude en su proyecto y ~/.claude en su directorio de inicio.
...
En Windows, `~/.claude` se resuelve a `%USERPROFILE%\.claude`. Si establece [`CLAUDE_CONFIG_DIR`](/docs/es/env-vars), cada ruta `~/.claude` en esta página vive bajo ese directorio en su lugar.
...
| ----------------------- | ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `managed-settings.json` | Nivel del sistema, varía según el SO | Configuración impuesta por la empresa que no puede anular. Consulte [configuración administrada por servidor](/docs/es/server-managed-settings).                                                                                                                                                                                     |
...
| :------------------------------------------------------------- | :-------------------------------------- | :---------------- | :------------------------------------------------- |
| Dar a Claude contexto del proyecto y convenciones              | `CLAUDE.md`                             | proyecto o global | [Memory](/docs/es/memory)                               |
| Permitir o bloquear llamadas de herramientas específicas       | `settings.json` `permissions` o `hooks` | proyecto o global | [Permissions](/docs/es/permissions), [Hooks](/docs/es/hooks) |
| Ejecutar un script antes o después de llamadas de herramientas | `settings.json` `hooks`                 | proyecto o global | [Hooks](/docs/es/hooks)                                 |
| Establecer variables de entorno para la sesión                 | `settings.json` `env`                   | proyecto o global | [Settings](/docs/es/settings#available-settings)        |
| Mantener anulaciones personales fuera de git                   | `settings.local.json`                   | solo proyecto     | [Settings scopes](/docs/es/settings#settings-files)     |
| Agregar un prompt o capacidad que invoque con `/name`          | `skills/<name>/SKILL.md`                | proyecto o global | [Skills](/docs/es/skills)                               |
| Definir un subagent especializado con sus propias herramientas | `agents/*.md`                           | proyecto o global | [Subagents](/docs/es/sub-agents)                        |
| Orquestar muchos subagents desde un script                     | `workflows/*.js`                        | proyecto o global | [Dynamic workflows](/docs/es/workflows)                 |
| Conectar herramientas externas sobre MCP                       | `.mcp.json`                             | solo proyecto     | [MCP](/docs/es/mcp)                                     |
| Cambiar cómo Claude formatea respuestas                        | `output-styles/*.md`                    | proyecto o global | [Output styles](/docs/es/output-styles)                 |
...
<Note>
  ...
  * [Configuración administrada](/docs/es/server-managed-settings) implementada por su organización tiene prioridad sobre todo
  * Las banderas CLI como `--permission-mode` o `--settings` anulan `settings.json` para esa sesión
  * Algunas variables de entorno tienen prioridad sobre su configuración equivalente, pero esto varía: consulte la [referencia de variables de entorno](/docs/es/env-vars) para cada una

  Consulte [precedencia de configuración](/docs/es/settings#settings-precedence) para el orden completo.
</Note>
...
| --------------------------------------------------- | ----------------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------- |
| [`CLAUDE.md`](#ce-claude-md)                        | Proyecto y global | ✓         | Instrucciones cargadas cada sesión                                                                                                         | [Memory](/docs/es/memory)                                            |
| [`rules/*.md`](#ce-rules)                           | Proyecto y global | ✓         | Instrucciones con alcance de tema, opcionalmente con puerta de ruta                                                                        | [Rules](/docs/es/memory#organize-rules-with-claude/rules/)           |
| [`settings.json`](#ce-settings-json)                | Proyecto y global | ✓         | Permisos, hooks, variables de entorno, valores predeterminados de modelo                                                                   | [Settings](/docs/es/settings)                                        |
| [`settings.local.json`](#ce-settings-local-json)    | Solo proyecto     |           | Sus anulaciones personales, auto-gitignored                                                                                                | [Settings scopes](/docs/es/settings#settings-files)                  |
| [`.mcp.json`](#ce-mcp-json)                         | Solo proyecto     | ✓         | Servidores MCP compartidos por el equipo                                                                                                   | [MCP scopes](/docs/es/mcp#mcp-installation-scopes)                   |
| [`.worktreeinclude`](#ce-worktreeinclude)           | Solo proyecto     | ✓         | Archivos ignorados por Git para copiar en nuevos worktrees                                                                                 | [Worktrees](/docs/es/worktrees#copy-gitignored-files-into-worktrees) |
| [`skills/<name>/SKILL.md`](#ce-skills)              | Proyecto y global | ✓         | Prompts reutilizables invocados con `/name` o auto-invocados                                                                               | [Skills](/docs/es/skills)                                            |
| [`commands/*.md`](#ce-commands)                     | Proyecto y global | ✓         | Prompts de archivo único; mismo mecanismo que skills                                                                                       | [Skills](/docs/es/skills)                                            |
| [`output-styles/*.md`](#ce-output-styles)           | Proyecto y global | ✓         | Secciones de prompt del sistema personalizadas                                                                                             | [Output styles](/docs/es/output-styles)                              |
| [`agents/*.md`](#ce-agents)                         | Proyecto y global | ✓         | Definiciones de subagents con su propio prompt y herramientas                                                                              | [Subagents](/docs/es/sub-agents)                                     |
| [`workflows/*.js`](#ce-workflows)                   | Proyecto y global | ✓         | Scripts de flujo de trabajo dinámico escritos por Claude y guardados desde `/workflows`; cada archivo se convierte en un comando `/<name>` | [Dynamic workflows](/docs/es/workflows)                              |
| [`agent-memory/<name>/`](#ce-agent-memory)          | Proyecto y global | ✓         | Memoria persistente para subagents                                                                                                         | [Persistent memory](/docs/es/sub-agents#enable-persistent-memory)    |
| [`~/.claude.json`](#ce-claude-json)                 | Solo global       |           | Estado de la aplicación, OAuth, alternancias de UI, servidores MCP personales                                                              | [Global config](/docs/es/settings#global-config-settings)            |
| [`projects/<project>/memory/`](#ce-global-projects) | Solo global       |           | Auto memory: notas de Claude para sí mismo entre sesiones                                                                                  | [Auto memory](/docs/es/memory#auto-memory)                           |
| [`keybindings.json`](#ce-keybindings)               | Solo global       |           | Atajos de teclado personalizados                                                                                                           | [Keybindings](/docs/es/keybindings)                                  |
| [`themes/*.json`](#ce-themes)                       | Solo global       |           | Temas de color personalizados                                                                                                              | [Custom themes](/docs/es/terminal-config#create-a-custom-theme)      |

<h2 id="troubleshoot-configuration">
  Solucionar problemas de configuración
</h2>

Si una configuración, hook o archivo no está surtiendo efecto, consulte [Depurar su configuración](/docs/es/debug-your-config) para los comandos de inspección y una tabla de búsqueda por síntoma.
...
Los archivos en las rutas a continuación se eliminan al inicio una vez que tienen más de [`cleanupPeriodDays`](/docs/es/settings#available-settings). El valor predeterminado es 30 días.
...
| `projects/<project>/<session>/subagents/`    | Transcripciones de conversación de [Subagent](/docs/es/sub-agents), eliminadas con la transcripción de sesión principal cuando vence                                                                                                                                                                                                   |
| `projects/<project>/<session>/tool-results/` | Salidas de herramientas grandes derramadas en archivos separados                                                                                                                                                                                                                                                                  |
| `file-history/<session>/`                    | Instantáneas previas a la edición de archivos que Claude cambió, utilizadas para [restauración de checkpoint](/docs/es/checkpointing). Contiene instantáneas para los 100 checkpoints más recientes; los archivos de instantánea que ningún checkpoint retenido referencia se eliminan, excepto la primera instantánea de cada archivo |
| `plans/`                                     | Archivos de plan escritos durante [plan mode](/docs/es/permission-modes#analyze-before-you-edit-with-plan-mode)                                                                                                                                                                                                                        |
...
| `shell-snapshots/`                           | Alias, funciones y opciones de shell capturadas al inicio y aplicadas por la [herramienta Bash](/docs/es/tools-reference#bash-tool-behavior) a cada comando. Se elimina al salir correctamente. El barrido borra cualquiera dejado después de un bloqueo.                                                                              |
| `backups/`                                   | Copias con marca de tiempo de `~/.claude.json` tomadas antes de migraciones de configuración                                                                                                                                                                                                                                      |
| `feedback-bundles/`                          | Archivos de transcripción redactados escritos por `/feedback` en proveedores de terceros o cuando no hay credenciales de Anthropic configuradas, para enviar a su equipo de cuenta de Anthropic                                                                                                                                   |
| `todos/`, `statsig/`, `logs/`                | Directorios heredados de versiones anteriores. Ya no se escriben. El barrido elimina su contenido y luego el directorio vacío.                                                                                                                                                                                                    |
...
| `remote-settings.json` | Copia en caché de [configuración administrada por servidor](/docs/es/server-managed-settings) para su organización. Solo presente cuando su organización las ha configurado. Se actualiza en cada inicio. |
...
* Establezca la variable de entorno [`CLAUDE_CODE_SKIP_PROMPT_HISTORY`](/docs/es/env-vars) para omitir la escritura de transcripciones e historial de prompts en cualquier modo. En modo no interactivo, puede pasar `--no-session-persistence` junto con `-p`, o establecer `persistSession: false` en el Agent SDK.
* Use [reglas de permisos](/docs/es/permissions) para denegar lecturas de archivos de credenciales

<h3 id="clear-local-data">
  Borrar datos locales
</h3>

Ejecute `claude project purge` para eliminar el estado que Claude Code mantiene para un proyecto. El comando requiere Claude Code v2.1.124 o posterior. Elimina:
...
| `~/.claude/todos/`, `~/.claude/statsig/`, `~/.claude/logs/`                                                                                                                                  | Nada. Directorios heredados no escritos por versiones actuales. |
...
* [Manage Claude's memory](/docs/es/memory): escriba y organice CLAUDE.md, rules y auto memory
* [Configure settings](/docs/es/settings): establezca permisos, hooks, variables de entorno y valores predeterminados de modelo
* [Create skills](/docs/es/skills): construya prompts y flujos de trabajo reutilizables
* [Configure subagents](/docs/es/sub-agents): defina agentes especializados con su propio contexto
