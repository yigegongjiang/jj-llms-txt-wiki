...
# Explorar la ventana de contexto

> Una simulación interactiva de cómo se llena la ventana de contexto de Claude Code durante una sesión. Vea qué se carga automáticamente, cuánto cuesta cada lectura de archivo y cuándo se activan las reglas y hooks.
...
Claude Code se compacta automáticamente a medida que se acerca al límite, por lo que una ventana de contexto llena no termina su sesión. El paso automático funciona de la misma manera que el paso `/compact` en la línea de tiempo. Vea [Cuando el contexto se llena](/docs/es/how-claude-code-works#when-context-fills-up) para saber qué preserva.
...
Para una cobertura más profunda de las características mostradas en la línea de tiempo, vea estas páginas:

* [Extender Claude Code](/docs/es/features-overview): cuándo usar CLAUDE.md vs skills vs reglas vs hooks vs MCP
* [Almacenar instrucciones y memorias](/docs/es/memory): jerarquía de CLAUDE.md y memoria automática
* [Subagents](/docs/es/sub-agents): delegar investigación a una ventana de contexto separada
* [Mejores prácticas](/docs/es/best-practices): gestionar el contexto como su restricción principal
* [Almacenamiento en caché de prompts](/docs/es/prompt-caching): qué acciones invalidan el prefijo en caché
* [Reducir el uso de tokens](/docs/es/costs#reduce-token-usage): estrategias para mantener el uso de contexto bajo
