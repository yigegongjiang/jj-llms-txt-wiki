...
decode_chunk_size (`int`, *optional*, defaults to 16) : The number of frames to decode at a time when calling `decode_latents` method.
...
#### __call__[[diffusers.HunyuanDiTPAGPipeline.__call__]]
...
return_dict (`bool`, *optional*, defaults to `True`) : Whether or not to return a `~pipelines.kolors.KolorsPipelineOutput` instead of a plain tuple.
...
**Returns:** `~pipelines.kolors.KolorsPipelineOutput` or `tuple`
...
image (`torch.Tensor`, `PIL.Image.Image`, `np.ndarray`, `list[torch.Tensor]`, `list[PIL.Image.Image]`, or `list[np.ndarray]`) : `Image`, numpy array or tensor representing an image batch to be inpainted (which parts of the image to be masked out with `mask_image` and repainted according to `prompt`).
...
strength (`float`, *optional*, defaults to 0.9999) : Indicates extent to transform the reference `image`. Must be between 0 and 1. `image` is used as a starting point and more noise is added the higher the `strength`. The number of denoising steps depends on the amount of noise initially added. When `strength` is 1, added noise is maximum and the denoising process runs for the full number of iterations specified in `num_inference_steps`. A value of 1 essentially ignores `image`.
...
#### __call__[[diffusers.StableDiffusionControlNetPAGInpaintPipeline.__call__]]
...
#### __call__[[diffusers.StableDiffusion3PAGPipeline.__call__]]
...
**Returns:** `~pipelines.stable_diffusion_3.StableDiffusion3PipelineOutput` or `tuple`
...
#### __call__[[diffusers.StableDiffusion3PAGImg2ImgPipeline.__call__]]
...
height (`int`, *optional*, defaults to `self.default_sample_size * self.vae_scale_factor`) : The height in pixels of the generated image. This is set to 1024 by default for the best results.

width (`int`, *optional*, defaults to `self.default_sample_size * self.vae_scale_factor`) : The width in pixels of the generated image. This is set to 1024 by default for the best results.
...
#### __call__[[diffusers.PixArtSigmaPAGPipeline.__call__]]
