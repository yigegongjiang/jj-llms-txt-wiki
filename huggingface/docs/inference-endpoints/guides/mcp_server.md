...
1. Click  **Setup MCP** in your Endpoints account menu, or click here.
...
Creating and updating endpoints requires a full or partial endpoint configuration object, which can be deeply nested depending on the engine and provider. Rather than writing one by hand, call get_recommended_config first for the model you want to deploy, then pass its output straight into create_endpoint.
