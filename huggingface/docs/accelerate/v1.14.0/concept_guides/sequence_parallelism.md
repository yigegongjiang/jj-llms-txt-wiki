...
See also the very related [Context Parallelism](./context_parallelism).
...
In the document [Context Parallelism](./context_parallelism) you can learn about deploying another technology called Context Parallelism, which too slices on the sequence dimension but uses Ring Attention instead of slicing on the head dimension.
