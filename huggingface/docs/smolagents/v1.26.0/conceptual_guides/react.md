...
- [CodeAgent](/docs/smolagents/v1.26.0/en/reference/agents#smolagents.CodeAgent) generates its tool calls as Python code snippets.
- [ToolCallingAgent](/docs/smolagents/v1.26.0/en/reference/agents#smolagents.ToolCallingAgent) writes its tool calls as JSON, as is common in many frameworks. Depending on your needs, either approach can be used. For instance, web browsing often requires waiting after each page interaction, so JSON tool calls can fit well.
