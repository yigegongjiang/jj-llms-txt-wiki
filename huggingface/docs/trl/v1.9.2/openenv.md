...
TRL's [GRPOTrainer](/docs/trl/v1.9.2/en/gspo_token#trl.GRPOTrainer) supports interactive environment training through the `environment_factory` argument. When provided, the trainer automatically handles the multi-turn tool-calling loop: it generates completions, parses tool calls, executes them against the environment, and feeds the results back to the model. All without custom rollout code.
...
[GRPOTrainer](/docs/trl/v1.9.2/en/gspo_token#trl.GRPOTrainer) supports two approaches for environment-based training:
