...
| [KTOTrainer](/docs/trl/v1.9.2/en/kto_trainer#trl.KTOTrainer) | [Unpaired preference](#unpaired-preference) or [Preference (explicit prompt recommended)](#preference) |
...
| [SFTTrainer](/docs/trl/v1.9.2/en/sft_trainer#trl.SFTTrainer) | [Language modeling](#language-modeling) or [Prompt-completion](#prompt-completion) |
| [experimental.bco.BCOTrainer](/docs/trl/v1.9.2/en/bco_trainer#trl.experimental.bco.BCOTrainer) | [Unpaired preference](#unpaired-preference) or [Preference (explicit prompt recommended)](#preference) |
...
| [experimental.ppo.PPOTrainer](/docs/trl/v1.9.2/en/ppo_trainer#trl.experimental.ppo.PPOTrainer) | Tokenized language modeling |
...
To convert a preference dataset with implicit prompt into a prompt-completion dataset, extract the prompt with [extract_prompt()](/docs/trl/v1.9.2/en/data_utils#trl.extract_prompt), remove the rejected, and rename the column `"chosen"` to `"completion"`.
...
To convert a preference dataset with implicit prompt into a prompt-only dataset, extract the prompt with [extract_prompt()](/docs/trl/v1.9.2/en/data_utils#trl.extract_prompt), and remove the rejected and the chosen.
...
To convert a preference dataset with implicit prompt into a preference dataset with explicit prompt, extract the prompt with [extract_prompt()](/docs/trl/v1.9.2/en/data_utils#trl.extract_prompt).
...
To convert a preference dataset with implicit prompt into an unpaired preference dataset, extract the prompt with [extract_prompt()](/docs/trl/v1.9.2/en/data_utils#trl.extract_prompt), and unpair the dataset with [unpair_preference_dataset()](/docs/trl/v1.9.2/en/data_utils#trl.unpair_preference_dataset).
...
> Before applying [unpair_preference_dataset()](/docs/trl/v1.9.2/en/data_utils#trl.unpair_preference_dataset), please ensure that all `"chosen"` completions can be labeled as good and all `"rejected"` completions as bad.
...
To convert dataset into an unpaired preference dataset, unpair the dataset with [unpair_preference_dataset()](/docs/trl/v1.9.2/en/data_utils#trl.unpair_preference_dataset).
