...
Pass these [TrainingArguments](/docs/transformers/v5.14.0/en/main_classes/trainer#transformers.TrainingArguments) to control DDP behavior.

- `gradient_accumulation_steps()` determines when to perform the all-reduce. [Trainer](/docs/transformers/v5.14.0/en/main_classes/trainer#transformers.Trainer) skips the all-reduce on intermediate accumulation steps and runs it only on the final micro-batch. For example, with `gradient_accumulation_steps=4`, the all-reduce runs every 4 backward passes.
