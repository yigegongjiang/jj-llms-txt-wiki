...
Create a PEFT config, like `LoraConfig` for example, and attach it to a model with [add_adapter()](/docs/transformers/v5.14.0/en/main_classes/peft#transformers.integrations.PeftAdapterMixin.add_adapter).
...
Pass the model with an attached adapter to [Trainer](/docs/transformers/v5.14.0/en/main_classes/trainer#transformers.Trainer) and call [train()](/docs/transformers/v5.14.0/en/main_classes/trainer#transformers.Trainer.train). [Trainer](/docs/transformers/v5.14.0/en/main_classes/trainer#transformers.Trainer) only updates the adapter parameters (those with `requires_grad=True`) because the base model is frozen.
...
During training, [Trainer](/docs/transformers/v5.14.0/en/main_classes/trainer#transformers.Trainer) checkpoints contain only the adapter weights (`adapter_model.safetensors`) and configuration (`adapter_config.json`), keeping checkpoints small. The base model isn't included.

After training, save the final adapter with [save_pretrained()](/docs/transformers/v5.14.0/en/main_classes/model#transformers.PreTrainedModel.save_pretrained).
...
[Trainer](/docs/transformers/v5.14.0/en/main_classes/trainer#transformers.Trainer) automatically detects adapter checkpoints when resuming. [Trainer](/docs/transformers/v5.14.0/en/main_classes/trainer#transformers.Trainer) scans the checkpoint directory for subdirectories containing adapter weights and reloads each adapter with the correct trainable state.
...
For ZeRO-3, [Trainer](/docs/transformers/v5.14.0/en/main_classes/trainer#transformers.Trainer) passes `exclude_frozen_parameters=True` when saving checkpoints with a PEFT model. Frozen base model weights are skipped. Only the trainable adapter parameters are saved, reducing checkpoint size and save time.

For FSDP, [Trainer](/docs/transformers/v5.14.0/en/main_classes/trainer#transformers.Trainer) updates the FSDP auto-wrap policy to correctly handle LoRA layers. For QLoRA (quantized base model + LoRA), [Trainer](/docs/transformers/v5.14.0/en/main_classes/trainer#transformers.Trainer) also adjusts the mixed precision policy to match the quantization storage dtype.
...
[from_pretrained()](/docs/transformers/v5.14.0/en/main_classes/model#transformers.PreTrainedModel.from_pretrained) automatically detects adapters. When it finds an `adapter_config.json`, it reads the `base_model_name_or_path` field to load the correct base model, then loads the adapter on top.
...
To load an adapter onto an existing model, use [load_adapter()](/docs/transformers/v5.14.0/en/main_classes/peft#transformers.integrations.PeftAdapterMixin.load_adapter).
...
Use [set_adapter()](/docs/transformers/v5.14.0/en/main_classes/peft#transformers.integrations.PeftAdapterMixin.set_adapter) to activate a specific adapter. The other adapters are disabled but remain in memory.

```py
model.set_adapter("adapter_2")
```

[enable_adapters()](/docs/transformers/v5.14.0/en/main_classes/peft#transformers.integrations.PeftAdapterMixin.enable_adapters) enables all attached adapters, and [disable_adapters()](/docs/transformers/v5.14.0/en/main_classes/peft#transformers.integrations.PeftAdapterMixin.disable_adapters) disables all of them.
...
Use [active_adapters()](/docs/transformers/v5.14.0/en/main_classes/peft#transformers.integrations.PeftAdapterMixin.active_adapters) to see which adapters are currently active.
...
Remove adapters you no longer need with [delete_adapter()](/docs/transformers/v5.14.0/en/main_classes/peft#transformers.integrations.PeftAdapterMixin.delete_adapter) to free memory.
...
For compiled models, call [enable_peft_hotswap()](/docs/transformers/v5.14.0/en/main_classes/peft#transformers.integrations.PeftAdapterMixin.enable_peft_hotswap) *before* loading the first adapter and before compiling.
