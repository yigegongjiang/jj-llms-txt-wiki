...
An optimizer updates model weights during training. The scheduler wraps the optimizer and adjusts the learning rate each training step. [Trainer](/docs/transformers/v5.14.0/en/main_classes/trainer#transformers.Trainer) creates both when it calls [create_optimizer_and_scheduler()](/docs/transformers/v5.14.0/en/main_classes/trainer#transformers.Trainer.create_optimizer_and_scheduler).
...
Configure optimizer and scheduler behavior, like `lr_scheduler_type()` and `optim()`, in [TrainingArguments](/docs/transformers/v5.14.0/en/main_classes/trainer#transformers.TrainingArguments). The defaults (`adamw_torch` optimizer and `linear` warmup scheduler) are a good starting point for most fine-tuning runs.
...
See the [GreedyLR](/docs/transformers/v5.14.0/en/main_classes/optimizer_schedules#transformers.GreedyLR) class for the full list of configurable parameters.
...
Set `optim` in [trl.SFTConfig](https://huggingface.co/docs/trl/v1.8.0/en/sft_trainer#trl.SFTConfig) to a GaLore optimizer (`"galore_adamw"`, `"galore_adafactor"`, or `"galore_adamw_8bit"`). Specify target modules with `optim_target_modules` and GaLore-specific parameters (`rank`, `update_proj_gap`, `scale`) through `optim_args`.
...
`~Trainer.optimizer_cls_and_kwargs` accepts a custom optimizer class while delegating parameter grouping and device placement to [Trainer](/docs/transformers/v5.14.0/en/main_classes/trainer#transformers.Trainer).

[Trainer](/docs/transformers/v5.14.0/en/main_classes/trainer#transformers.Trainer) defers building the optimizer until [create_optimizer()](/docs/transformers/v5.14.0/en/main_classes/trainer#transformers.Trainer.create_optimizer) runs, so the model is already on the correct device.
...
Pass a predefined optimizer and scheduler to `~Trainer.optimizers`. [Trainer](/docs/transformers/v5.14.0/en/main_classes/trainer#transformers.Trainer) skips [create_optimizer()](/docs/transformers/v5.14.0/en/main_classes/trainer#transformers.Trainer.create_optimizer) and [create_scheduler()](/docs/transformers/v5.14.0/en/main_classes/trainer#transformers.Trainer.create_scheduler) when prebuilt instances are provided. If you don't pass a scheduler, [Trainer](/docs/transformers/v5.14.0/en/main_classes/trainer#transformers.Trainer) automatically creates one.
...
Prebuilt instances bypass [create_optimizer()](/docs/transformers/v5.14.0/en/main_classes/trainer#transformers.Trainer.create_optimizer) and [create_scheduler()](/docs/transformers/v5.14.0/en/main_classes/trainer#transformers.Trainer.create_scheduler), so you need to specify your own parameter groups.

### Override optimizer and scheduler methods

Subclass [create_optimizer()](/docs/transformers/v5.14.0/en/main_classes/trainer#transformers.Trainer.create_optimizer) and [create_scheduler()](/docs/transformers/v5.14.0/en/main_classes/trainer#transformers.Trainer.create_scheduler) for full control. Both methods run *during* [train()](/docs/transformers/v5.14.0/en/main_classes/trainer#transformers.Trainer.train).

Override [create_scheduler()](/docs/transformers/v5.14.0/en/main_classes/trainer#transformers.Trainer.create_scheduler) to use a scheduler like [OneCycleLR](https://docs.pytorch.org/docs/stable/generated/torch.optim.lr_scheduler.OneCycleLR.html) that isn't available in [SchedulerType](/docs/transformers/v5.14.0/en/main_classes/optimizer_schedules#transformers.SchedulerType).
