...
- If you're using Transformers < 4.37.0.dev, set `trust_remote_code=True` in [from_pretrained()](/docs/transformers/v5.14.0/en/model_doc/auto#transformers.AutoModel.from_pretrained). Otherwise, make sure you update Transformers to the latest stable version.
...
This is the configuration class to store the configuration of a PhiModel. It is used to instantiate a Phi
model according to the specified arguments, defining the model architecture. Instantiating a configuration with the
defaults will yield a similar configuration to that of the [microsoft/phi-1](https://huggingface.co/microsoft/phi-1)
...
- **config** ([PhiConfig](/docs/transformers/v5.14.0/en/model_doc/phi#transformers.PhiConfig)) --
...
elements depending on the configuration ([PhiConfig](/docs/transformers/v5.14.0/en/model_doc/phi#transformers.PhiConfig)) and inputs.
The [PhiModel](/docs/transformers/v5.14.0/en/model_doc/phi#transformers.PhiModel) forward method, overrides the `__call__` special method.
...
- **config** ([PhiForCausalLM](/docs/transformers/v5.14.0/en/model_doc/phi#transformers.PhiForCausalLM)) --
  Model configuration class with all the parameters of the model. Initializing with a config file does not
  load the weights associated with the model, only the configuration. Check out the
  [from_pretrained()](/docs/transformers/v5.14.0/en/main_classes/model#transformers.PreTrainedModel.from_pretrained) method to load the model weights.
...
`torch.FloatTensor` (if `return_dict=False` is passed or when `config.return_dict=False`) comprising various
elements depending on the configuration ([PhiConfig](/docs/transformers/v5.14.0/en/model_doc/phi#transformers.PhiConfig)) and inputs.
The [PhiForCausalLM](/docs/transformers/v5.14.0/en/model_doc/phi#transformers.PhiForCausalLM) forward method, overrides the `__call__` special method.
