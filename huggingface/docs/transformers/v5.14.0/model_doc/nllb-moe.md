...
- **hidden_states** (`torch.Tensor`) --
  (batch_size, sequence_length, hidden_dim) from which router probabilities are computed.top_1_mask (`torch.Tensor` of shape (batch_size, sequence_length))Index tensor of shape [batch_size, sequence_length] corresponding to the expert selected for each token
...
- **config** ([NllbMoeConfig](/docs/transformers/v5.14.0/en/model_doc/nllb-moe#transformers.NllbMoeConfig)) --
...
- **use_cache** (`bool`, *optional*) --
  ...
  `past_key_values`).`Seq2SeqMoEModelOutput` or `tuple(torch.FloatTensor)`A `Seq2SeqMoEModelOutput` or a tuple of
`torch.FloatTensor` (if `return_dict=False` is passed or when `config.return_dict=False`) comprising various
elements depending on the configuration (`None`) and inputs.
The [NllbMoeModel](/docs/transformers/v5.14.0/en/model_doc/nllb-moe#transformers.NllbMoeModel) forward method, overrides the `__call__` special method.
...
`torch.FloatTensor` (if `return_dict=False` is passed or when `config.return_dict=False`) comprising various
elements depending on the configuration ([NllbMoeConfig](/docs/transformers/v5.14.0/en/model_doc/nllb-moe#transformers.NllbMoeConfig)) and inputs.
The [NllbMoeForConditionalGeneration](/docs/transformers/v5.14.0/en/model_doc/nllb-moe#transformers.NllbMoeForConditionalGeneration) forward method, overrides the `__call__` special method.
