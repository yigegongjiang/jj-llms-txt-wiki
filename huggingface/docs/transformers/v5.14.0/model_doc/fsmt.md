...
- FSMT uses source and target vocabulary pairs that aren't combined into one. It doesn't share embeddings tokens
  either. Its tokenizer is very similar to [XLMTokenizer](/docs/transformers/v5.14.0/en/model_doc/xlm#transformers.XLMTokenizer) and the main model is derived from
  [BartModel](/docs/transformers/v5.14.0/en/model_doc/bart#transformers.BartModel).

## FSMTConfig[[transformers.FSMTConfig]]

- **is_encoder_decoder** (`bool`, *optional*, defaults to `True`) --
  Whether the model is used as an encoder/decoder or not.
- **langs** (`Union[list[str], tuple[str, ...]]`, *optional*, defaults to `('en', 'de')`) --
  A list with source language and target_language (e.g., ['en', 'ru']).
- **src_vocab_size** (`int`, *optional*, defaults to `42024`) --
  Vocabulary size of the encoder. Defines the number of different tokens that can be represented by the
  `inputs_ids` passed to the forward method in the encoder.
- **tgt_vocab_size** (`int`, *optional*, defaults to `42024`) --
...
'"}, {"name": "bos_token", "val": " = ''"}, {"name": "sep_token", "val": " = ''"}, {"name": "pad_token", "val": " = ''"}, {"name": "**kwargs", "val": ""}]}>
...
- **config** ([FSMTConfig](/docs/transformers/v5.14.0/en/model_doc/fsmt#transformers.FSMTConfig)) --
...
elements depending on the configuration ([FSMTConfig](/docs/transformers/v5.14.0/en/model_doc/fsmt#transformers.FSMTConfig)) and inputs.
The [FSMTModel](/docs/transformers/v5.14.0/en/model_doc/fsmt#transformers.FSMTModel) forward method, overrides the `__call__` special method.
...
`torch.FloatTensor` (if `return_dict=False` is passed or when `config.return_dict=False`) comprising various
elements depending on the configuration ([FSMTConfig](/docs/transformers/v5.14.0/en/model_doc/fsmt#transformers.FSMTConfig)) and inputs.
The [FSMTForConditionalGeneration](/docs/transformers/v5.14.0/en/model_doc/fsmt#transformers.FSMTForConditionalGeneration) forward method, overrides the `__call__` special method.
