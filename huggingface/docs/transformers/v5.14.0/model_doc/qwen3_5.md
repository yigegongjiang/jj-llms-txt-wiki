...
- Layers are hybrid: [Qwen3_5TextConfig](/docs/transformers/v5.14.0/en/model_doc/qwen3_5#transformers.Qwen3_5TextConfig)'s `layer_types` is a per-layer list of `"linear_attention"` or `"full_attention"` that encodes the 3:1 Gated DeltaNet / Gated Attention stack. The DeltaNet path (`Qwen3NextGatedDeltaNet`) needs the optional `causal_conv1d` (from [Dao-AILab](https://github.com/Dao-AILab/causal-conv1d)) and `fla` packages for its fast kernels — without them, the model silently falls back to slower and more memory hungry PyTorch ops.
- On NVIDIA GB10 (compute capability 12.1 / SM121) neither `causal_conv1d` nor `fla` ship an SM121 build, so the DeltaNet path always falls back to the slow PyTorch reference. Passing `use_kernels=True` (`pip install -U kernels`) to [from_pretrained()](/docs/transformers/v5.14.0/en/main_classes/model#transformers.PreTrainedModel.from_pretrained) swaps the Gated DeltaNet conv1d and delta-rule cores for a compute-capability-gated Hub kernel ([`Atlas-Inference/gdn`](https://huggingface.co/kernels/Atlas-Inference/gdn)); every other GPU keeps the existing path. The kernel is numerically faithful to the fallback (identical greedy output) and speeds up prefill. Measured on `Qwen/Qwen3.6-27B` (bf16, GB10/SM121, 1024-token prompt, greedy decode of 256 tokens):
...
- Use [Qwen3_5ForCausalLM](/docs/transformers/v5.14.0/en/model_doc/qwen3_5#transformers.Qwen3_5ForCausalLM) for text-only generation with [Qwen3_5TextConfig](/docs/transformers/v5.14.0/en/model_doc/qwen3_5#transformers.Qwen3_5TextConfig); use [Qwen3_5ForConditionalGeneration](/docs/transformers/v5.14.0/en/model_doc/qwen3_5#transformers.Qwen3_5ForConditionalGeneration) with the full [Qwen3_5Config](/docs/transformers/v5.14.0/en/model_doc/qwen3_5#transformers.Qwen3_5Config) and a processor ([from_pretrained()](/docs/transformers/v5.14.0/en/model_doc/auto#transformers.AutoProcessor.from_pretrained)) to feed interleaved image/video + text via [apply_chat_template()](/docs/transformers/v5.14.0/en/main_classes/processors#transformers.ProcessorMixin.apply_chat_template).

## Qwen3_5Config[[transformers.Qwen3_5Config]]
...
This is the configuration class to store the configuration of a Qwen3_5Model. It is used to instantiate a Qwen3 5
model according to the specified arguments, defining the model architecture. Instantiating a configuration with the
defaults will yield a similar configuration to that of the [Qwen/Qwen3.5-27B](https://huggingface.co/Qwen/Qwen3.5-27B)
...
## Qwen3_5VisionConfig[[transformers.Qwen3_5VisionConfig]]
...
## Qwen3_5Tokenizer[[transformers.Qwen3_5Tokenizer]]

'"}, {"name": "bos_token", "val": " = None"}, {"name": "eos_token", "val": ": str = '<|endoftext|>'"}, {"name": "pad_token", "val": ": str = '<|endoftext|>'"}, {"name": "add_prefix_space", "val": " = None"}, {"name": "**kwargs", "val": ""}]}>

## Qwen3_5VisionModel[[transformers.Qwen3_5VisionModel]]

- **hidden_states** (`torch.Tensor` of shape `(seq_len, hidden_size)`) --
  The final hidden states of the model.
- **grid_thw** (`torch.Tensor` of shape `(num_images_or_videos, 3)`) --
  The temporal, height and width of feature shape of each image in LLM.`torch.Tensor`hidden_states.

## Qwen3_5TextModel[[transformers.Qwen3_5TextModel]]
...
elements depending on the configuration ([Qwen3_5Config](/docs/transformers/v5.14.0/en/model_doc/qwen3_5#transformers.Qwen3_5Config)) and inputs.
The [Qwen3_5TextModel](/docs/transformers/v5.14.0/en/model_doc/qwen3_5#transformers.Qwen3_5TextModel) forward method, overrides the `__call__` special method.
...
## Qwen3_5Model[[transformers.Qwen3_5Model]]

- **config** ([Qwen3_5Model](/docs/transformers/v5.14.0/en/model_doc/qwen3_5#transformers.Qwen3_5Model)) --
...
- **mm_token_type_ids** (`torch.IntTensor` of shape `(batch_size, sequence_length)`, *optional*) --
  ...
  Multimodal token type ids can be obtained using [AutoProcessor](/docs/transformers/v5.14.0/en/model_doc/auto#transformers.AutoProcessor). See [ProcessorMixin.__call__()](/docs/transformers/v5.14.0/en/main_classes/processors#transformers.ProcessorMixin.__call__) for details.`Qwen3_5ModelOutputWithPast` or `tuple(torch.FloatTensor)`A `Qwen3_5ModelOutputWithPast` or a tuple of
`torch.FloatTensor` (if `return_dict=False` is passed or when `config.return_dict=False`) comprising various
elements depending on the configuration (`None`) and inputs.
The [Qwen3_5Model](/docs/transformers/v5.14.0/en/model_doc/qwen3_5#transformers.Qwen3_5Model) forward method, overrides the `__call__` special method.
...
## Qwen3_5ForCausalLM[[transformers.Qwen3_5ForCausalLM]]

- **config** ([Qwen3_5ForCausalLM](/docs/transformers/v5.14.0/en/model_doc/qwen3_5#transformers.Qwen3_5ForCausalLM)) --
  Model configuration class with all the parameters of the model. Initializing with a config file does not
  load the weights associated with the model, only the configuration. Check out the
  [from_pretrained()](/docs/transformers/v5.14.0/en/main_classes/model#transformers.PreTrainedModel.from_pretrained) method to load the model weights.
...
`torch.FloatTensor` (if `return_dict=False` is passed or when `config.return_dict=False`) comprising various
elements depending on the configuration ([Qwen3_5Config](/docs/transformers/v5.14.0/en/model_doc/qwen3_5#transformers.Qwen3_5Config)) and inputs.
The [Qwen3_5ForCausalLM](/docs/transformers/v5.14.0/en/model_doc/qwen3_5#transformers.Qwen3_5ForCausalLM) forward method, overrides the `__call__` special method.
...
## Qwen3_5ForSequenceClassification[[transformers.Qwen3_5ForSequenceClassification]]

## Qwen3_5TextForSequenceClassification[[transformers.Qwen3_5TextForSequenceClassification]]
...
## Qwen3_5ForTokenClassification[[transformers.Qwen3_5ForTokenClassification]]
