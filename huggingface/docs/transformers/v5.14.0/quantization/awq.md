...
Create an [AwqConfig](/docs/transformers/v5.14.0/en/main_classes/quantization#transformers.AwqConfig) and set the parameters `fuse_max_seq_len` and `do_fuse=True` to enable fused modules. The `fuse_max_seq_len` parameter is the total sequence length and it should include the context length and the expected generation length. Set it to a larger value to be safe.
...
For architectures that don't support fused modules, create an [AwqConfig](/docs/transformers/v5.14.0/en/main_classes/quantization#transformers.AwqConfig) and define a custom fusing mapping in `modules_to_fuse` to determine which modules need to be fused.
...
Set `version="exllama"` in [AwqConfig](/docs/transformers/v5.14.0/en/main_classes/quantization#transformers.AwqConfig) to enable ExLlamaV2 kernels.
