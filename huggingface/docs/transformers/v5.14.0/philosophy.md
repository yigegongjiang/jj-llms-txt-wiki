...
- On top of those three base classes, the library provides two APIs: [pipeline()](/docs/transformers/v5.14.0/en/main_classes/pipelines#transformers.pipeline) for quickly
    using a model for inference on a given task and [Trainer](/docs/transformers/v5.14.0/en/main_classes/trainer#transformers.Trainer) to quickly train or fine-tune a PyTorch model.
...
- **Model classes** are PyTorch models ([torch.nn.Module](https://pytorch.org/docs/stable/nn.html#torch.nn.Module)), wrapped by at least a [PreTrainedModel](/docs/transformers/v5.14.0/en/main_classes/model#transformers.PreTrainedModel).
