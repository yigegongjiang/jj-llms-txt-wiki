...
embedding_multiplier (`float`, *optional*, defaults to `1.0`) : Scaling factor applied to the token embedding outputs. Used in MuP to control the scale of the embedding activations.

logits_scaling (`float`, *optional*, defaults to `1.0`) : Scaling factor **multiplied** to the final logits before loss computation or sampling. Used in MuP to ensure consistent output scale across model sizes. Note: unlike [GraniteConfig](/docs/transformers/v5.15.1/en/model_doc/granite#transformers.GraniteConfig), this is a multiplier, not a divisor.

residual_multiplier (`float`, *optional*, defaults to `1.0`) : Scaling factor applied to each sub-layer output before adding to the residual stream. Used in Maximal Update Parametrization (MuP) to stabilize training across model sizes.

attention_multiplier (`float`, *optional*, defaults to `head_dim ** -0.5`) : Scaling factor applied to attention logits before softmax, replacing the standard `1 / sqrt(head_dim)` scaling. Set explicitly for MuP-based training; when `None`, defaults to the standard value.

head_dim (`int`, *optional*) : The attention head dimension. If None, it will default to hidden_size // num_attention_heads

use_post_norm (`bool`, *optional*, defaults to `True`) : Whether to apply an extra RMSNorm after each sub-layer output (Peri-Layer Normalization).

This is the configuration class to store the configuration of a HyperCLOVAXModel. It is used to instantiate a Hyperclovax
model according to the specified arguments, defining the model architecture. Instantiating a configuration with the
defaults will yield a similar configuration to that of the [naver-hyperclovax/HyperCLOVAX-SEED-Think-14B](https://huggingface.co/naver-hyperclovax/HyperCLOVAX-SEED-Think-14B)
...
#### validate_architecture[[transformers.HyperCLOVAXConfig.validate_architecture]]

```python
validate_architecture()
```

[Source](https://github.com/huggingface/transformers/blob/v5.15.1/src/transformers/models/hyperclovax/configuration_hyperclovax.py#L125)

Validates that `hidden_size` is divisible by `num_attention_heads`.

## HyperCLOVAXModel[[transformers.HyperCLOVAXModel]]

#### transformers.HyperCLOVAXModel[[transformers.HyperCLOVAXModel]]

```python
transformers.HyperCLOVAXModel(config: HyperCLOVAXConfig)
```

[Source](https://github.com/huggingface/transformers/blob/v5.15.1/src/transformers/models/hyperclovax/modeling_hyperclovax.py#L369)

**Parameters:**

config ([HyperCLOVAXConfig](/docs/transformers/v5.15.1/en/model_doc/hyperclovax#transformers.HyperCLOVAXConfig)) : Model configuration class with all the parameters of the model. Initializing with a config file does not load the weights associated with the model, only the configuration. Check out the [from_pretrained()](/docs/transformers/v5.15.1/en/main_classes/model#transformers.PreTrainedModel.from_pretrained) method to load the model weights.
...
#### forward[[transformers.HyperCLOVAXModel.forward]]

```python
forward(input_ids: typing.Optional[torch.LongTensor] = None, attention_mask: typing.Optional[torch.Tensor] = None, position_ids: typing.Optional[torch.LongTensor] = None, past_key_values: transformers.cache_utils.Cache | None = None, inputs_embeds: typing.Optional[torch.FloatTensor] = None, use_cache: bool | None = None, **kwargs: Unpack)
```

[Source](https://github.com/huggingface/transformers/blob/v5.15.1/src/transformers/models/hyperclovax/modeling_hyperclovax.py#L387)
...
elements depending on the configuration ([HyperCLOVAXConfig](/docs/transformers/v5.15.1/en/model_doc/hyperclovax#transformers.HyperCLOVAXConfig)) and inputs.

The [HyperCLOVAXModel](/docs/transformers/v5.15.1/en/model_doc/hyperclovax#transformers.HyperCLOVAXModel) forward method, overrides the `__call__` special method.
...
## HyperCLOVAXForCausalLM[[transformers.HyperCLOVAXForCausalLM]]

#### transformers.HyperCLOVAXForCausalLM[[transformers.HyperCLOVAXForCausalLM]]

```python
transformers.HyperCLOVAXForCausalLM(config)
```

[Source](https://github.com/huggingface/transformers/blob/v5.15.1/src/transformers/models/hyperclovax/modeling_hyperclovax.py#L447)

**Parameters:**

config ([HyperCLOVAXForCausalLM](/docs/transformers/v5.15.1/en/model_doc/hyperclovax#transformers.HyperCLOVAXForCausalLM)) : Model configuration class with all the parameters of the model. Initializing with a config file does not load the weights associated with the model, only the configuration. Check out the [from_pretrained()](/docs/transformers/v5.15.1/en/main_classes/model#transformers.PreTrainedModel.from_pretrained) method to load the model weights.

The Hyperclovax Model for causal language modeling.
...
#### forward[[transformers.HyperCLOVAXForCausalLM.forward]]
...
[Source](https://github.com/huggingface/transformers/blob/v5.15.1/src/transformers/models/hyperclovax/modeling_hyperclovax.py#L462)
...
The [HyperCLOVAXForCausalLM](/docs/transformers/v5.15.1/en/model_doc/hyperclovax#transformers.HyperCLOVAXForCausalLM) forward method, overrides the `__call__` special method.
