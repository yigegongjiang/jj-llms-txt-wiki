...
#### preprocess[[transformers.DetrImageProcessor.preprocess]]
...
#### transformers.DetrImageProcessorPil[[transformers.DetrImageProcessorPil]]
...
#### preprocess[[transformers.DetrImageProcessorPil.preprocess]]
...
#### post_process_object_detection[[transformers.DetrImageProcessorPil.post_process_object_detection]]
...
loss (`torch.FloatTensor` of shape `(1,)`, *optional*, returned when `labels` are provided)) : Total loss as a linear combination of a negative log-likelihood (cross-entropy) for class prediction and a bounding box loss. The latter is defined as a linear combination of the L1 loss and the generalized scale-invariant IoU loss.
...
#### forward[[transformers.DetrModel.forward]]
...
inputs_embeds (`torch.FloatTensor` of shape `(batch_size, sequence_length, hidden_size)`, *optional*) : Optionally, instead of passing the flattened feature map (output of the backbone + projection layer), you can choose to directly pass a flattened representation of an image. Useful for bypassing the vision backbone.
...
#### forward[[transformers.DetrForObjectDetection.forward]]
...
#### forward[[transformers.DetrForSegmentation.forward]]
