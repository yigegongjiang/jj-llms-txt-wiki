...
```python
transformers.SmolVLMVisionTransformer(config: SmolVLMVisionConfig)
```

[Source](https://github.com/huggingface/transformers/blob/v5.15.1/src/transformers/models/smolvlm/modeling_smolvlm.py#L305)
...
```python
transformers.SmolVLMModel(config: SmolVLMConfig)
```

[Source](https://github.com/huggingface/transformers/blob/v5.15.1/src/transformers/models/smolvlm/modeling_smolvlm.py#L437)
...
#### forward[[transformers.SmolVLMModel.forward]]
...
[Source](https://github.com/huggingface/transformers/blob/v5.15.1/src/transformers/models/smolvlm/modeling_smolvlm.py#L563)
...
pixel_values (`torch.FloatTensor` of shape `(batch_size, num_channels, image_size, image_size)`, *optional*) : The tensors corresponding to the input images. Pixel values can be obtained using [SmolVLMImageProcessor](/docs/transformers/v5.15.1/en/model_doc/smolvlm#transformers.SmolVLMImageProcessor). See `SmolVLMImageProcessor.__call__()` for details ([SmolVLMProcessor](/docs/transformers/v5.15.1/en/model_doc/smolvlm#transformers.SmolVLMProcessor) uses [SmolVLMImageProcessor](/docs/transformers/v5.15.1/en/model_doc/smolvlm#transformers.SmolVLMImageProcessor) for processing images).
...
**Returns:** `SmolVLMBaseModelOutputWithPast` or `tuple(torch.FloatTensor)`

A `SmolVLMBaseModelOutputWithPast` or a tuple of
`torch.FloatTensor` (if `return_dict=False` is passed or when `config.return_dict=False`) comprising various
elements depending on the configuration ([SmolVLMConfig](/docs/transformers/v5.15.1/en/model_doc/smolvlm#transformers.SmolVLMConfig)) and inputs.
...
[Source](https://github.com/huggingface/transformers/blob/v5.15.1/src/transformers/models/smolvlm/modeling_smolvlm.py#L507)
...
```python
transformers.SmolVLMForConditionalGeneration(config)
```

[Source](https://github.com/huggingface/transformers/blob/v5.15.1/src/transformers/models/smolvlm/modeling_smolvlm.py#L680)
...
#### forward[[transformers.SmolVLMForConditionalGeneration.forward]]
...
[Source](https://github.com/huggingface/transformers/blob/v5.15.1/src/transformers/models/smolvlm/modeling_smolvlm.py#L717)
...
**Returns:** `SmolVLMCausalLMOutputWithPast` or `tuple(torch.FloatTensor)`

A `SmolVLMCausalLMOutputWithPast` or a tuple of
`torch.FloatTensor` (if `return_dict=False` is passed or when `config.return_dict=False`) comprising various
elements depending on the configuration ([SmolVLMConfig](/docs/transformers/v5.15.1/en/model_doc/smolvlm#transformers.SmolVLMConfig)) and inputs.

The [SmolVLMForConditionalGeneration](/docs/transformers/v5.15.1/en/model_doc/smolvlm#transformers.SmolVLMForConditionalGeneration) forward method, overrides the `__call__` special method.
...
[Source](https://github.com/huggingface/transformers/blob/v5.15.1/src/transformers/models/smolvlm/modeling_smolvlm.py#L700)
...
```python
transformers.SmolVLMImageProcessor(**kwargs: Unpack)
```

[Source](https://github.com/huggingface/transformers/blob/v5.15.1/src/transformers/models/smolvlm/image_processing_smolvlm.py#L191)
...
size (`Annotated[int | list[int] | tuple[int, ...] | dict[str, int] | None, None]`, *kwargs*, defaults to `{'longest_edge' : 1456}`): Describes the maximum input dimensions to the model.
...
do_image_splitting (`bool`, *kwargs*, *optional*, defaults to `True`) : Whether to split the image into sub-images concatenated with the original image. They are split into patches such that each patch has a size of `max_image_size["height"]` x `max_image_size["width"]`.

max_image_size (`Dict`, *kwargs*, *optional*, defaults to `{"longest_edge" : 364}`): Maximum resolution of the patches of images accepted by the model. This is a dictionary containing the key "longest_edge".

return_row_col_info (`bool`, *kwargs*, *optional*, defaults to `False`) : Whether to return the row and column information of the images.

Constructs a SmolVLMImageProcessor image processor.

#### preprocess[[transformers.SmolVLMImageProcessor.preprocess]]

```python
preprocess(images: typing.Union[ForwardRef('PIL.Image.Image'), numpy.ndarray, ForwardRef('torch.Tensor'), list['PIL.Image.Image'], list[numpy.ndarray], list['torch.Tensor']], **kwargs: Unpack)
```

[Source](https://github.com/huggingface/transformers/blob/v5.15.1/src/transformers/models/smolvlm/image_processing_smolvlm.py#L210)
...
#### transformers.SmolVLMImageProcessorPil[[transformers.SmolVLMImageProcessorPil]]

```python
transformers.SmolVLMImageProcessorPil(**kwargs: Unpack)
```

[Source](https://github.com/huggingface/transformers/blob/v5.15.1/src/transformers/models/smolvlm/image_processing_pil_smolvlm.py#L190)
...
#### preprocess[[transformers.SmolVLMImageProcessorPil.preprocess]]

```python
preprocess(images: typing.Union[ForwardRef('PIL.Image.Image'), numpy.ndarray, ForwardRef('torch.Tensor'), list['PIL.Image.Image'], list[numpy.ndarray], list['torch.Tensor']], **kwargs: Unpack)
```

[Source](https://github.com/huggingface/transformers/blob/v5.15.1/src/transformers/models/smolvlm/image_processing_pil_smolvlm.py#L209)
...
#### transformers.SmolVLMVideoProcessor[[transformers.SmolVLMVideoProcessor]]

```python
transformers.SmolVLMVideoProcessor(**kwargs: Unpack)
```

[Source](https://github.com/huggingface/transformers/blob/v5.15.1/src/transformers/models/smolvlm/video_processing_smolvlm.py#L104)

#### preprocess[[transformers.SmolVLMVideoProcessor.preprocess]]
...
#### transformers.SmolVLMProcessor[[transformers.SmolVLMProcessor]]

```python
transformers.SmolVLMProcessor(image_processor, tokenizer, video_processor, image_seq_len: int = 169, chat_template: str | None = None, **kwargs)
```

[Source](https://github.com/huggingface/transformers/blob/v5.15.1/src/transformers/models/smolvlm/processing_smolvlm.py#L110)

**Parameters:**

image_processor (`SmolVLMImageProcessor`) : The image processor is a required input.

tokenizer (`TokenizersBackend`) : The tokenizer is a required input.

video_processor (`SmolVLMVideoProcessor`) : The video processor is a required input.
