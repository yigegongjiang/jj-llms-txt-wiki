...
embedding_dim (`int`, *optional*) : Dimensionality of the embeddings and hidden states.
...
#### forward[[transformers.xLSTMModel.forward]]
...
cache_params (`xLSTMCache`, *optional*) : The xLSTMCache that carries the RNN states.

use_cache (`bool`, *optional*) : If set to `True`, `past_key_values` key value states are returned and can be used to speed up decoding (see `past_key_values`).

**Returns:** `xLSTMOutput` or `tuple(torch.FloatTensor)`
...
#### forward[[transformers.xLSTMForCausalLM.forward]]
...
**Returns:** `xLSTMCausalLMOutput` or `tuple(torch.FloatTensor)`
