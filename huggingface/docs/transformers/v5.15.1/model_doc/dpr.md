...
projection_dim (`int`, *optional*, defaults to `0`) : Dimensionality of text and vision projection layers.
...
**Returns:** `dict[str, list[list[int]]]`
...
#### forward[[transformers.DPRContextEncoder.forward]]
...
input_ids (`torch.LongTensor` of shape `(batch_size, sequence_length)`) : Indices of input sequence tokens in the vocabulary. To match pretraining, DPR input sequence should be formatted with [CLS] and [SEP] tokens as follows:  (a) For sequence pairs (for a pair title+text for example):
...
#### forward[[transformers.DPRQuestionEncoder.forward]]
...
#### forward[[transformers.DPRReader.forward]]
