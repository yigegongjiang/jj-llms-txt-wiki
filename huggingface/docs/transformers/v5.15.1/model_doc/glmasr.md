...
audio_token_id (`int`, *optional*, defaults to `59260`) : The audio token index used as a placeholder for input audio.
...
```python
transformers.GlmAsrPreTrainedModel(config: PreTrainedConfig, *inputs, **kwargs)
```

[Source](https://github.com/huggingface/transformers/blob/v5.15.1/src/transformers/models/glmasr/modeling_glmasr.py#L274)
...
#### forward[[transformers.GlmAsrPreTrainedModel.forward]]
...
```python
transformers.GlmAsrProcessor(feature_extractor, tokenizer, chat_template = None, audio_token = '<|pad|>', default_transcription_prompt = 'Please transcribe this audio into text', max_audio_len = 655)
```

[Source](https://github.com/huggingface/transformers/blob/v5.15.1/src/transformers/models/glmasr/processing_glmasr.py#L58)
...
#### __call__[[transformers.GlmAsrProcessor.__call__]]
...
[Source](https://github.com/huggingface/transformers/blob/v5.15.1/src/transformers/models/glmasr/processing_glmasr.py#L85)
...
```python
transformers.GlmAsrEncoder(config: GlmAsrEncoderConfig)
```

[Source](https://github.com/huggingface/transformers/blob/v5.15.1/src/transformers/models/glmasr/modeling_glmasr.py#L287)

#### forward[[transformers.GlmAsrEncoder.forward]]

```python
forward(input_features, **kwargs: Unpack)
```

[Source](https://github.com/huggingface/transformers/blob/v5.15.1/src/transformers/models/glmasr/modeling_glmasr.py#L310)
...
The [GlmAsrEncoder](/docs/transformers/v5.15.1/en/model_doc/glmasr#transformers.GlmAsrEncoder) forward method, overrides the `__call__` special method.
...
#### transformers.GlmAsrModel[[transformers.GlmAsrModel]]

```python
transformers.GlmAsrModel(config)
```

[Source](https://github.com/huggingface/transformers/blob/v5.15.1/src/transformers/models/glmasr/modeling_glmasr.py#L364)

**Parameters:**

config ([GlmAsrModel](/docs/transformers/v5.15.1/en/model_doc/glmasr#transformers.GlmAsrModel)) : Model configuration class with all the parameters of the model. Initializing with a config file does not load the weights associated with the model, only the configuration. Check out the [from_pretrained()](/docs/transformers/v5.15.1/en/main_classes/model#transformers.PreTrainedModel.from_pretrained) method to load the model weights.
...
#### forward[[transformers.GlmAsrModel.forward]]
...
[Source](https://github.com/huggingface/transformers/blob/v5.15.1/src/transformers/models/glmasr/modeling_glmasr.py#L434)
...
**Returns:** `GlmAsrModelOutputWithPast` or `tuple(torch.FloatTensor)`

A `GlmAsrModelOutputWithPast` or a tuple of
`torch.FloatTensor` (if `return_dict=False` is passed or when `config.return_dict=False`) comprising various
elements depending on the configuration ([GlmAsrConfig](/docs/transformers/v5.15.1/en/model_doc/glmasr#transformers.GlmAsrConfig)) and inputs.

The [GlmAsrModel](/docs/transformers/v5.15.1/en/model_doc/glmasr#transformers.GlmAsrModel) forward method, overrides the `__call__` special method.
...
```python
transformers.GlmAsrForConditionalGeneration(config)
```

[Source](https://github.com/huggingface/transformers/blob/v5.15.1/src/transformers/models/glmasr/modeling_glmasr.py#L516)
...
#### forward[[transformers.GlmAsrForConditionalGeneration.forward]]
...
[Source](https://github.com/huggingface/transformers/blob/v5.15.1/src/transformers/models/glmasr/modeling_glmasr.py#L528)
...
input_features_mask (`torch.Tensor` of shape `(batch_size, feature_sequence_length)`) : Mask to avoid performing attention on padding feature indices. Mask values selected in `[0, 1]`:  - 1 for tokens that are **not masked**, - 0 for tokens that are **masked**.
...
The [GlmAsrForConditionalGeneration](/docs/transformers/v5.15.1/en/model_doc/glmasr#transformers.GlmAsrForConditionalGeneration) forward method, overrides the `__call__` special method.
