...
final_dropout (`float`, *optional*, defaults to 0.1) : The dropout probability for the final projection layer of [SEWForCTC](/docs/transformers/v5.15.1/en/model_doc/sew#transformers.SEWForCTC).
...
This is the configuration class to store the configuration of a SEWModel. It is used to instantiate a Sew
...
## SEWModel[[transformers.SEWModel]]

#### transformers.SEWModel[[transformers.SEWModel]]

```python
transformers.SEWModel(config: SEWConfig)
```

[Source](https://github.com/huggingface/transformers/blob/v5.15.1/src/transformers/models/sew/modeling_sew.py#L686)
...
The bare Sew Model outputting raw hidden-states without any specific head on top.
...
#### forward[[transformers.SEWModel.forward]]
...
[Source](https://github.com/huggingface/transformers/blob/v5.15.1/src/transformers/models/sew/modeling_sew.py#L753)
...
elements depending on the configuration ([SEWConfig](/docs/transformers/v5.15.1/en/model_doc/sew#transformers.SEWConfig)) and inputs.

The [SEWModel](/docs/transformers/v5.15.1/en/model_doc/sew#transformers.SEWModel) forward method, overrides the `__call__` special method.
...
## SEWForCTC[[transformers.SEWForCTC]]

#### transformers.SEWForCTC[[transformers.SEWForCTC]]

```python
transformers.SEWForCTC(config, target_lang: str | None = None)
```

[Source](https://github.com/huggingface/transformers/blob/v5.15.1/src/transformers/models/sew/modeling_sew.py#L817)
...
target_lang (`str`, *optional*) : Language id of adapter weights. Adapter weights are stored in the format adapter..safetensors or adapter..bin. Only relevant when using an instance of [SEWForCTC](/docs/transformers/v5.15.1/en/model_doc/sew#transformers.SEWForCTC) with adapters. Uses 'eng' by default.
...
#### forward[[transformers.SEWForCTC.forward]]
...
[Source](https://github.com/huggingface/transformers/blob/v5.15.1/src/transformers/models/sew/modeling_sew.py#L886)
...
The [SEWForCTC](/docs/transformers/v5.15.1/en/model_doc/sew#transformers.SEWForCTC) forward method, overrides the `__call__` special method.
...
>>> model = SEWForCTC.from_pretrained("BAAI/seggpt-vit-large")
...
```python
transformers.SEWForSequenceClassification(config)
```

[Source](https://github.com/huggingface/transformers/blob/v5.15.1/src/transformers/models/sew/modeling_sew.py#L965)
...
#### forward[[transformers.SEWForSequenceClassification.forward]]

```python
forward(input_values: typing.Optional[torch.Tensor], attention_mask: typing.Optional[torch.Tensor] = None, output_attentions: bool | None = None, output_hidden_states: bool | None = None, return_dict: bool | None = None, labels: typing.Optional[torch.Tensor] = None, **kwargs)
```

[Source](https://github.com/huggingface/transformers/blob/v5.15.1/src/transformers/models/sew/modeling_sew.py#L998)

**Parameters:**

input_values (`torch.FloatTensor` of shape `(batch_size, sequence_length)`) : Float values of input raw speech waveform. Values can be obtained by loading a `.flac` or `.wav` audio file into an array of type `list[float]`, a `numpy.ndarray` or a `torch.Tensor`, *e.g.* via the torchcodec library (`pip install torchcodec`) or the soundfile library (`pip install soundfile`). To prepare the array into `input_values`, the [AutoProcessor](/docs/transformers/v5.15.1/en/model_doc/auto#transformers.AutoProcessor) should be used for padding and conversion into a tensor of type `torch.FloatTensor`. See `SEWProcessor.__call__` for details.
...
The [SEWForSequenceClassification](/docs/transformers/v5.15.1/en/model_doc/sew#transformers.SEWForSequenceClassification) forward method, overrides the `__call__` special method.
...
>>> model = SEWForSequenceClassification.from_pretrained("BAAI/seggpt-vit-large")
...
>>> model = SEWForSequenceClassification.from_pretrained("BAAI/seggpt-vit-large", num_labels=num_labels)
...
>>> tokenizer = AutoTokenizer.from_pretrained("BAAI/seggpt-vit-large")
>>> model = SEWForSequenceClassification.from_pretrained("BAAI/seggpt-vit-large", problem_type="multi_label_classification")
