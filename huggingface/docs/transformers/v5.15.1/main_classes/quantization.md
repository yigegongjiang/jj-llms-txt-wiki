...
#### post_init[[transformers.QuantoConfig.post_init]]
...
#### post_init[[transformers.AqlmConfig.post_init]]
...
#### post_init[[transformers.VptqConfig.post_init]]
...
#### post_init[[transformers.EetqConfig.post_init]]
...
#### from_dict_optimum[[transformers.GPTQConfig.from_dict_optimum]]
...
#### is_quantizable[[transformers.BitsAndBytesConfig.is_quantizable]]
...
#### adjust_max_memory[[transformers.quantizers.HfQuantizer.adjust_max_memory]]
...
#### post_init[[transformers.HiggsConfig.post_init]]
...
#### from_dict[[transformers.HqqConfig.from_dict]]
...
format (`str`, *optional*, defaults to `"dense"`) : format the weights are stored in, e.g. `dense`, `float-quantized` or `pack-quantized`
...
run_compressed (`bool`, *optional*) : deprecated, use `dequantize` instead. Its only effect is to set `dequantize` to its opposite, `run_compressed=False` meaning `dequantize=True`.
...
#### from_dict[[transformers.CompressedTensorsConfig.from_dict]]
...
**Returns:** `QuantizationConfigMixin`
...
#### from_dict[[transformers.TorchAoConfig.from_dict]]
...
#### post_init[[transformers.BitNetQuantConfig.post_init]]
...
#### post_init[[transformers.SpQRConfig.post_init]]
...
scale_fmt (`str`, *optional*, defaults to `"float"`) : Storage dtype of the per-block weight scales: `"float"` (fp32, V3-style) or `"ue8m0"` (1-byte `torch.float8_e8m0fnu`, V4-style).

FineGrainedFP8Config is a configuration class for fine-grained FP8 quantization used mainly for deepseek models.

#### post_init[[transformers.FineGrainedFP8Config.post_init]]
...
#### post_init[[transformers.FPQuantConfig.post_init]]
...
#### post_init[[transformers.AutoRoundConfig.post_init]]
