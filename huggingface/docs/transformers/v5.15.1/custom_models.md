...
A configuration, given by the base [PreTrainedConfig](/docs/transformers/v5.15.1/en/main_classes/configuration#transformers.PreTrainedConfig) class, contains all the necessary information to build a model. This is where you'll configure the attributes of the custom ResNet model. Different attributes gives different ResNet model types.

The main rules for customizing a configuration are:

1. A custom configuration must subclass [PreTrainedConfig](/docs/transformers/v5.15.1/en/main_classes/configuration#transformers.PreTrainedConfig). This ensures a custom model has all the functionality of a Transformers' model such as [from_pretrained()](/docs/transformers/v5.15.1/en/main_classes/configuration#transformers.PreTrainedConfig.from_pretrained), [save_pretrained()](/docs/transformers/v5.15.1/en/main_classes/configuration#transformers.PreTrainedConfig.save_pretrained), and [push_to_hub()](/docs/transformers/v5.15.1/en/main_classes/model#transformers.utils.PushToHubMixin.push_to_hub).
2. The [PreTrainedConfig](/docs/transformers/v5.15.1/en/main_classes/configuration#transformers.PreTrainedConfig) `__init__` must accept any `kwargs` and they must be passed to the superclass `__init__`. [PreTrainedConfig](/docs/transformers/v5.15.1/en/main_classes/configuration#transformers.PreTrainedConfig) has more fields than the ones set in your custom configuration, so when you load a configuration with [from_pretrained()](/docs/transformers/v5.15.1/en/main_classes/configuration#transformers.PreTrainedConfig.from_pretrained), those fields need to be accepted by your configuration and passed to the superclass.
...
Save the configuration to a JSON file in your custom model folder, `custom-resnet`, with [save_pretrained()](/docs/transformers/v5.15.1/en/main_classes/configuration#transformers.PreTrainedConfig.save_pretrained).
...
With the custom ResNet configuration, you can now create and customize the model. The model subclasses the base [PreTrainedModel](/docs/transformers/v5.15.1/en/main_classes/model#transformers.PreTrainedModel) class. Like [PreTrainedConfig](/docs/transformers/v5.15.1/en/main_classes/configuration#transformers.PreTrainedConfig), inheriting from [PreTrainedModel](/docs/transformers/v5.15.1/en/main_classes/model#transformers.PreTrainedModel) and initializing the superclass with the configuration extends Transformers' functionalities such as saving and loading to the custom model.
...
> [!TIP]
> The first argument to [AutoConfig.register()](/docs/transformers/v5.15.1/en/model_doc/auto#transformers.AutoConfig.register) must match the `model_type` attribute in the custom configuration class, and the first argument to `AutoModel.register()` must match the `config_class` of the custom model class.
...
Your custom model code is now compatible with the [AutoClass](./models#autoclass) API. Users can load the model with the [AutoModel](./model_doc/auto#automodel) or [AutoModelForImageClassification](/docs/transformers/v5.15.1/en/model_doc/auto#transformers.AutoModelForImageClassification) classes.
...
Copy the code from the model and configuration files. To make sure the AutoClass objects are saved with [save_pretrained()](/docs/transformers/v5.15.1/en/main_classes/model#transformers.PreTrainedModel.save_pretrained), call the [register_for_auto_class()](/docs/transformers/v5.15.1/en/main_classes/configuration#transformers.PreTrainedConfig.register_for_auto_class) method. This modifies the configuration JSON file to include the AutoClass objects and mapping.
...
Call [push_to_hub()](/docs/transformers/v5.15.1/en/main_classes/model#transformers.utils.PushToHubMixin.push_to_hub) on the model to upload the model to the Hub.
...
Because a custom model doesn't use the same modeling code as a Transformers' model, you need to add `trust_remote_code=True` in [from_pretrained()](/docs/transformers/v5.15.1/en/main_classes/model#transformers.PreTrainedModel.from_pretrained) to load it. Refer to the load [custom models](./models#custom-models) section for more information.
