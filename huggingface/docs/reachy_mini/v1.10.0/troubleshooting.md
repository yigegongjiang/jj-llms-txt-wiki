...
**Wireless Reachy Mini**: run `reachyminios_check` to make sure everything is fine (see [Get Started](./platforms/reachy_mini/get_started))  

Bootstrap or update fails / Python environment issues (Lite & Simulation)
...
If you have any of the following symptoms, please refer to the [Motors diagnosis and troubleshooting guide](./troubleshooting/motors_diagnosis) for detailed steps to diagnose and fix motor-related issues.  
...
If your cable is plugged properly and you still have issues, it is likely that the FPC cable of the microphone is damaged. Please refer to the tutorial [How to change the FPC cable of the microphone of Reachy Mini?](./troubleshooting/change_mic_fpc_cable) to fix this issue.
...
[Getting Started](./platforms/reachy_mini/get_started)
...
Head makes squeaking or friction noises during movement
...
This is normal wear over time. Follow the [Spherical Joints Maintenance guide](./troubleshooting/spherical_joints_maintenance) to clean and re-grease the joints.
...
- With Reachy Mini Lite, you can use [the desktop app](./platforms/reachy_mini_lite/get_started).
...
See the [Reachy Mini Wireless guide](./platforms/reachy_mini/get_started) for detailed instructions on connecting to Wi-Fi.

How do I reset the Wi-Fi hotspot?

If you need to reset the robot's Wi-Fi hotspot (for example, if you can't connect or want to change the network), follow the instructions in the [Wi-Fi Reset Guide](./platforms/reachy_mini/reset).
...
Robot and computer can't communicate on conference/hotel WiFi
...
How to access to HuggingFace services from China?
...
How to make the conversation app work in China?
...
Refer to the [Reachy Mini Hardware page](./platforms/reachy_mini/hardware) for detailed information about the robot's hardware components.
...
```
2. Lite: You can also use the Dynamixel Wizard to read motors parameters. Follow the guide [here](./platforms/reachy_mini_lite/wizard). 
...
See the full guide: [Building & Publishing Apps](./SDK/apps) — covers app structure, testing, publishing, debugging, and deployment.

My app crashes silently or doesn't start
...
For more debugging tips (viewing logs, common pitfalls), see [Debugging Apps](./SDK/apps#debugging-apps).

Is installing apps directly from Reachy Mini Control supported?
...
The Web App can't connect to my Reachy
...
My robot doesn't move and no error is reported
...
Can I get the raw microphone output?
...
Motor '' hardware errors: ['Input Voltage Error']
...
Conversation app crashes on launch after update (custom profile)
