...
* **On Reachy Mini Lite (USB)** - You have two options :
  - Start the [desktop application](../platforms/reachy_mini_lite/get_started#3--download-reachy-mini-control)
  - Open a terminal and run :
    ```bash
    reachy-mini-daemon
    ```
* **For Simulation (No robot needed)** - You have two options :
  ...
  - Open a terminal and run:
    ...
    - **macOS:**
      ...
      >
      > If you get a segmentation fault from `libgstpython`, see the [simulation troubleshooting](../platforms/simulation/get_started#-troubleshooting) section.
