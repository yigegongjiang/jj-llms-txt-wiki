...
To create a new webhook, use [create_webhook()](/docs/huggingface_hub/v1.25.1/en/package_reference/hf_api#huggingface_hub.HfApi.create_webhook) and specify the URL where payloads should be sent, what events should be watched, and optionally set a domain and a secret for security.
...
To see all the webhooks you have configured, you can list them with [list_webhooks()](/docs/huggingface_hub/v1.25.1/en/package_reference/hf_api#huggingface_hub.HfApi.list_webhooks). This is useful to review their IDs, URLs, and statuses.
...
If you need to change the configuration of an existing webhook, such as the URL or the events it watches, you can update it using [update_webhook()](/docs/huggingface_hub/v1.25.1/en/package_reference/hf_api#huggingface_hub.HfApi.update_webhook).
...
You might want to temporarily disable a webhook without deleting it. This can be done using [disable_webhook()](/docs/huggingface_hub/v1.25.1/en/package_reference/hf_api#huggingface_hub.HfApi.disable_webhook), and the webhook can be re-enabled later with [enable_webhook()](/docs/huggingface_hub/v1.25.1/en/package_reference/hf_api#huggingface_hub.HfApi.enable_webhook).
...
When a webhook is no longer needed, it can be permanently deleted using [delete_webhook()](/docs/huggingface_hub/v1.25.1/en/package_reference/hf_api#huggingface_hub.HfApi.delete_webhook).
...
The base class that we will use in this guides section is [WebhooksServer()](/docs/huggingface_hub/v1.25.1/en/package_reference/webhooks_server#huggingface_hub.WebhooksServer). It is a class for easily configuring a server that
...
1. By decorating a function with [webhook_endpoint()](/docs/huggingface_hub/v1.25.1/en/package_reference/webhooks_server#huggingface_hub.webhook_endpoint), a [WebhooksServer()](/docs/huggingface_hub/v1.25.1/en/package_reference/webhooks_server#huggingface_hub.WebhooksServer) object has been created in the background.
...
automatically parse the payload and pass it to the function as a [WebhookPayload](/docs/huggingface_hub/v1.25.1/en/package_reference/webhooks_server#huggingface_hub.WebhookPayload) object. This is a `pydantic` object
...
The guide above explained the quickest way to setup a [WebhooksServer()](/docs/huggingface_hub/v1.25.1/en/package_reference/webhooks_server#huggingface_hub.WebhooksServer). In this section, we will see how to customize
it further.
...
To get more flexibility, you can also create a [WebhooksServer()](/docs/huggingface_hub/v1.25.1/en/package_reference/webhooks_server#huggingface_hub.WebhooksServer) object directly. This is useful if you want to
...
trigger the webhooks. When creating a [WebhooksServer()](/docs/huggingface_hub/v1.25.1/en/package_reference/webhooks_server#huggingface_hub.WebhooksServer), you can register new webhooks using the
...
2. We create a [WebhooksServer()](/docs/huggingface_hub/v1.25.1/en/package_reference/webhooks_server#huggingface_hub.WebhooksServer) object with a custom UI and a secret. The secret is optional and can be set with
