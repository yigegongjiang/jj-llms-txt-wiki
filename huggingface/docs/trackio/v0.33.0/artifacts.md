...
For finer control — multiple files, custom logical paths, a description, or metadata — construct an [Artifact](/docs/trackio/v0.33.0/en/api#trackio.Artifact), add files to it, then log it:
...
[use_artifact()](/docs/trackio/v0.33.0/en/api#trackio.use_artifact) fetches an artifact and records it as an **input** to the current run, which is what builds lineage between a consuming run and the artifact it used:
...
`use_artifact` returns an [Artifact](/docs/trackio/v0.33.0/en/api#trackio.Artifact) whose files you materialize with `download()`:
